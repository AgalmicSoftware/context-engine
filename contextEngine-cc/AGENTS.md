...
## Quick commands
```bash
npm install               # only dependency: ethers@5
npm start                 # server on http://localhost:7391
npm run dev               # server with --watch (auto-restart)
node --test test/*.test.mjs  # run all tests
node --test lib/*.test.mjs   # run lib-level tests
```
