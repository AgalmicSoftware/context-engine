...
This checkout does not currently track `server.mjs`, `package.json`, or
`public/js/sessionSlugs.mjs` under `contextEngine-cc/`, so package-local
`npm install`, `npm start`, and server-level HTTP tests are blocked until those
runtime assets are restored from the correct source or generated by a supported
workflow.
