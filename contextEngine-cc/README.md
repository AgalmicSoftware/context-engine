...
Current checkout note: this branch does not track `contextEngine-cc/server.mjs`,
`contextEngine-cc/package.json`, or `contextEngine-cc/public/js/sessionSlugs.mjs`.
Server-level CE-CC HTTP tests remain blocked until those runtime assets are
restored from the correct source or generated by a supported workflow. From the
repo root, `npm run test:cc` runs the dependency-free agent contract and router
harness tests that are meaningful in this package shape.
