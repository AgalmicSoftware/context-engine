...
export const WORKER_SETUP_STEPS = Object.freeze({
  ...
  TELEGRAM_WEBHOOK_CONFIGURED: 'telegram_webhook_configured',
  ...
  SBT_GATE_CHECKED: 'sbt_gate_checked',
  ...
  SUBMIT_REQUEST_CREATED: 'submit_request_created',
...
export function normalizeWorkerSetupSecretStatus(input = {}) {
  const fields = input.fields && typeof input.fields === 'object' ? input.fields : input;
  const names = ['telegramBotToken', 'webhookSecret', 'ceAgentToken', 'openclawToken'];
  return {
    type: 'agent_worker_setup_secret_status',
    version: WORKER_SETUP_CONTRACT_VERSION,
    saved: input.saved === true,
    fields: Object.fromEntries(names.map((name) => [name, {
      configured: safeString(fields?.[name]).length > 0 || fields?.[name]?.configured === true,
      displayValue: null,
      value: null,
    }])),
    secretsDisplayedAfterSave: false,
  };
}

export function normalizeWorkerSetupSessionStorageDisplay(input = {}) {
  const profile = lower(input.storageProfile || input.backend || 'arweave') === 'cloudflare' ? 'cloudflare' : 'arweave';
  return {
    type: 'agent_worker_setup_session_storage_display',
    version: WORKER_SETUP_CONTRACT_VERSION,
    storageProfile: profile,
    sessionOwned: true,
    telegramOwned: false,
    editableHere: false,
    policyRoute: '/new?mode=advanced',
    summary: profile === 'cloudflare'
      ? 'Cloudflare session storage profile selected in /new Advanced.'
      : 'Arweave session storage profile selected in /new Advanced.',
  };
}
...
export function normalizeWorkerSetupState(input = {}) {
  ...
  return {
    ...
    secrets: normalizeWorkerSetupSecretStatus(input.secrets || {}),
    sessionStorage: normalizeWorkerSetupSessionStorageDisplay(input.sessionStorage || input.storageProfile || {}),
    steps,
