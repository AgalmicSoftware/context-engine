...
describe('admin Arweave ports', () => {
  it('routes balance and upload calls through call-time Arweave script lookup', async () => {
    ...
    expect(firstScripts.readArweaveWalletBalance).toHaveBeenCalledWith(jwk, undefined);
    ...
    expect(secondScripts.buildArweaveGatewayUrl).toHaveBeenCalledWith('second-tx', undefined);
  });
