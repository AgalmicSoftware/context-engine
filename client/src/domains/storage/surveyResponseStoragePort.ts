...
>;
export type SurveyResponseStorageArweaveUrlModule = Pick<
  typeof arweaveUrls,
  'normalizeArweaveUrl'
>;
export type SurveyResponseStorageRefModule = Pick<
  typeof storageRefs,
...
export const bindSurveyResponseStoragePort = ({
  ...
  getLegacyArweaveTxId: (record, options) => (
    readStorageRefs().getLegacyArweaveTxId(record, options)
  ),
  normalizeArweaveUrl: (value, options) => (
    readArweaveUrls().normalizeArweaveUrl(value, options)
  ),
  buildQuestionArweaveHref: (question, options) => {
    const arweaveTxId = readStorageRefs().getLegacyArweaveTxId(question);
    return arweaveTxId
      ? readArweaveUrls().normalizeArweaveUrl(arweaveTxId, options)
      : '';
  },
});
