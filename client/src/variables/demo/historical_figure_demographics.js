...
const normalizeCountry = (value = '') => COUNTRY_OVERRIDES[String(value || '').trim()] || String(value || '').trim();

const normalizeEraBucket = (value = '') => {
  const normalized = String(value || '').trim();
  return normalized;
};

const buildMergedFigureLookup = () => {
...
const ADDITIONAL_FIGURE_LOOKUP = new Map(
  Object.entries(additionalHistoricalFigures || {}).map(([key, value]) => [String(key || '').trim(), value])
);

const HISTORICAL_USER_LOOKUP = new Map(
  (Array.isArray(historicalFigureUsers) ? historicalFigureUsers : [])
    .map((entry) => [String(entry?.username || '').trim(), entry])
    .filter(([key]) => key)
);

const buildSourceBackedEntry = (xid = '') => {
...
const buildCanonicalEntry = (xid = '') => {
  const manualEntry = MANUAL_DEMOGRAPHICS[xid];
  if (manualEntry) {
