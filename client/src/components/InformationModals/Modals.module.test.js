...
describe('Modals contrast styles', () => {
  ...
  it('keeps the intro image bottom-flush while preserving the centered titleless slide hooks', () => {
    ...
    expect(scss).toMatch(/\.welcomeSlideMediaButton\[data-slide-layout='flushBottom'\]\s*\{\s*overflow:\s*hidden;[\s\S]*?align-items:\s*flex-end;/);
    expect(scss).toMatch(/\.welcomeSlideEmbed \.welcomeSlideLayout \.welcomeSlideMediaButton\[data-slide-layout='flushBottom'\]\s*\{[\s\S]*?align-self:\s*stretch;[\s\S]*?height:\s*auto\s*!important;[\s\S]*?align-items:\s*flex-end;/);
    expect(scss).toMatch(/\.welcomeSlideImageIntro\[data-slide-layout='flushBottom'\]\s*\{[\s\S]*?max-height:\s*none\s*!important;[\s\S]*?object-fit:\s*contain;[\s\S]*?object-position:\s*left bottom;[\s\S]*?transform:\s*translateY\(2\.75%\);/);
    expect(scss).toMatch(/@media \(min-width:\s*1367px\)\s*{[\s\S]*?\.welcomeSlideImageIntro\[data-slide-layout='flushBottom'\]\s*\{[\s\S]*?width:\s*75%;[\s\S]*?height:\s*100%;/);
