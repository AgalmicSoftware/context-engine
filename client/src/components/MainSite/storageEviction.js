...
export function trimLargeArrays(obj, max = 500, seen) {
...
export function evictOldDgEntries(maxAgeMs = 7 * 24 * 60 * 60 * 1000) {
  let meta = {};
...
export function updateDgMetaTimestamp(storageKey) {
  try {
    let meta = {};
    try { meta = JSON.parse(localStorage.getItem(DG_META_STORAGE_KEY) || '{}'); } catch (e) { mainSiteLog.warn('MainSite: fallback', e); }
...
export function removeDgMetaTimestamp(storageKey) {
  try {
    let meta = {};
    try { meta = JSON.parse(localStorage.getItem(DG_META_STORAGE_KEY) || '{}'); } catch (e) { mainSiteLog.warn('MainSite: fallback', e); }
