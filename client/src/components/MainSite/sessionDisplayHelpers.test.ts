...
const createHeaderOpts = (overrides: Record<string, unknown> = {}, demoConfig: unknown = null) => ({
  ...
  normalizeSessionSlug,
  getDemoSessionConfigBySlug: jest.fn(() => demoConfig),
  normalizeArweaveUrl: jest.fn(
    (value: string) => `normalized:${String(value || '').trim()}`
  ),
});
...
describe('sessionDisplayHelpers', () => {
  describe('hasEncryptedSessionField', () => {
    it('returns true for encryptedFields.sessionName', () => {
      expect(hasEncryptedSessionField({
  ...
  describe('getSessionInfoForGroup', () => {
    it('returns an override when present for the normalized slug', () => {
      const opts = createTextOpts({ alpha: 'Override info' }, {
        sessionInfo: 'Demo info',
      });

      expect(getSessionInfoForGroup({ slug: ' Alpha ', sessionInfo: 'Config info' }, '', opts))
        .toBe('Override info');
    });
    ...
    it('cascades through config fields', () => {
      ...
      expect(getSessionInfoForGroup({ sessionInfo: 'Session info field' }, 'alpha', opts))
        .toBe('Session info field');
      expect(getSessionInfoForGroup({ info: 'Info field' }, 'alpha', opts)).toBe('Info field');
      expect(getSessionInfoForGroup({ description: 'Description field' }, 'alpha', opts))
        .toBe('Description field');
    });
  ...
  describe('getSessionNameForGroup', () => {
    it('returns an override when present for the normalized slug', () => {
      const opts = createTextOpts({ alpha: 'Override name' }, {
        sessionName: 'Demo name',
      });

      expect(getSessionNameForGroup({ slug: ' Alpha ', sessionName: 'Config name' }, '', opts))
        .toBe('Override name');
    });
    ...
    it('cascades through config fields', () => {
      ...
      expect(getSessionNameForGroup({ sessionName: 'Session name field' }, 'alpha', opts))
        .toBe('Session name field');
      expect(getSessionNameForGroup({ name: 'Name field' }, 'alpha', opts)).toBe('Name field');
      expect(getSessionNameForGroup({ title: 'Title field' }, 'alpha', opts))
        .toBe('Title field');
    });
  ...
  describe('getSessionHeaderForGroup', () => {
    it('returns a normalized arweave URL from an override', () => {
      ...
      expect(getSessionHeaderForGroup({ slug: 'Alpha' }, '', opts))
        .toBe('normalized:ar://override-header');
    ...
    it('cascades through config fields', () => {
      ...
      expect(getSessionHeaderForGroup({ headerImage: 'ar://header-image' }, 'alpha', opts))
        .toBe('normalized:ar://header-image');
      expect(getSessionHeaderForGroup({ header: 'ar://header' }, 'alpha', opts))
        .toBe('normalized:ar://header');
    });
