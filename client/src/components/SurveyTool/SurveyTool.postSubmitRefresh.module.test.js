...
} from './surveyToolSignatures';
import {
  getConvictionFromSlice,
  getImportanceFromSlice,
} from './surveyToolResponseState';
...
const buildSurveyPayload = (slice = buildSubmitState()) => buildResponsePayload({
  account: '0xabc',
  isStandalone: false,
  singleQuestionMode: false,
  surveyId: '0xsurvey',
...
const runPostSubmitFollowup = async ({
  ...
  const isSubmitContextCurrent = jest.fn(() => (
    currentChecks.length > 0 ? currentChecks.shift() : true
  ));
  const events = [];
...
describe('SurveyTool post-submit refresh', () => {
  ...
  it('skips immediate response refreshes after submit when local cache write-through succeeds', async () => {
    ...
    expect(outcome.result.statePatch).toEqual(expect.objectContaining({
    ...
    expect(writeSubmittedResponsesToLocalCaches).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('passes the merged encrypted slice into submit work before async state flush', async () => {
    ...
    expect(filtered.questionResponses[0].answer).toEqual(expect.objectContaining({
      encrypted: true,
      encryptedPortion: 'answer-env',
    }));
    expect(filtered.questionResponses[0].additional).toEqual(expect.objectContaining({
      encrypted: true,
      encryptedPortion: 'additional-env',
    }));
    expect(receipt.__ceQuestionResponses[0].answer).toEqual(expect.objectContaining({
      encryptedPortion: 'answer-env',
    }));
    expect(receipt.__ceSurveyResponse.responses[0].additional).toEqual(expect.objectContaining({
      encryptedPortion: 'additional-env',
    }));
  ...
  it('uses the resolved submission slug for post-submit cache writes and refresh fallback', async () => {
    ...
    expect(outcome.writeSubmittedResponsesToLocalCaches).toHaveBeenCalledWith(expect.objectContaining({
