...
export const createSurveyResultsLockedResponsesRuntime = ({
  ...
  const handleDecryptLockedResponses = async (): Promise<void> => {
    ...
    const nextOverrides: Record<string, SurveyResultsDecryptedResponseOverride> = {
      ...(toSurveyResultsRecord(ports.getState().decryptedResponseOverrides) as Record<string, SurveyResultsDecryptedResponseOverride>),
    };
    ...
    for (const row of lockedRows) {
      ...
      if (
        ...
        try {
          ...
          anyDecrypted = true;
        } catch (e) { ports.logWarn('SurveyResults: fallback', e); }
      }
    ...
    ports.applyStatePatch(buildSurveyResultsLockedResponsesDecryptCompletePatch({
      anyDecrypted,
  ...
  const toggleLockedResponseDetails = (): void => {
    ports.applyStatePatch((prevState: SurveyResultsState) => (
      toggleSurveyResultsLockedResponseDetailsPatch(prevState)
    ));
  };
