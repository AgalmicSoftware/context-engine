...
describe('surveyPileResponseWindow', () => {
  it('builds the centered pile response window and normalized signature', () => {
    expect(buildPileResponseWindow({
      pileQuestions,
      activePileIndex: 3,
    })).toEqual({
      startIdx: 1,
  ...
  it('initializes only the currently visible pile response slots', () => {
    ...
    expect(plan.initialSlice?.answers?.q2).toEqual(expect.objectContaining({
      value: '',
      fieldKey: 'answer',
      questionId: 'q2',
    }));
    expect(plan.initialSlice?.additionalComments?.q6).toEqual(expect.objectContaining({
      value: '',
      fieldKey: 'additional',
      questionId: 'q6',
    }));
  ...
  it('skips pile response initialization while dirty or when the visible signature is unchanged', () => {
    expect(buildInitializePileResponseStatePlan({
      pileQuestions,
      activePileIndex: 0,
      isDirty: true,
      buildEmptyResponseFieldState,
  ...
  it('backfills missing visible pile response and baseline slots without overwriting existing entries', () => {
    ...
    expect(plan.updates?.surveysResponseState?.[0]?.answers?.q2).toEqual(expect.objectContaining({
      fieldKey: 'answer',
      questionId: 'q2',
    }));
    expect(plan.updates?.surveysResponseState?.[0]?.additionalComments?.q3).toEqual(expect.objectContaining({
      fieldKey: 'additional',
      questionId: 'q3',
    }));
    expect(plan.updates?.editBaseline?.answers?.q4).toEqual(expect.objectContaining({
      fieldKey: 'answer',
      questionId: 'q4',
    }));
    expect(plan.updates?.editBaseline?.additionalComments?.q4).toEqual(expect.objectContaining({
      fieldKey: 'additional',
      questionId: 'q4',
    }));
  });
  ...
  it('returns a no-op plan when every visible pile card already has response slots', () => {
    ...
    expect(filledPlan).toEqual(expect.objectContaining({
      shouldSkip: true,
