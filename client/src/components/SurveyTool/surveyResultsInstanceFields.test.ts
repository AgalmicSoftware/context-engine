...
describe('surveyResultsInstanceFields', () => {
  it('creates a fresh instance bag with the expected memo defaults', () => {
    ...
    expect(first._lockedResponsesModelMemo).toEqual(expect.objectContaining({
      aggregatorRef: null,
      overridesRef: null,
    ...
    expect((first._scopedQuestionNetworkDataSyncMemo as Record<string, unknown>).result)
      .toBe(EMPTY_SCOPED_QUESTION_NETWORK_DATA);
  });
});
