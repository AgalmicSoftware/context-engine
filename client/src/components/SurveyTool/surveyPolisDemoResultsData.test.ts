...
const countQuestionResponses = (questionResponses: Record<string, Record<string, unknown>>): number => (
  Object.values(questionResponses).reduce(
    (sum, responders) => sum + Object.keys(responders || {}).length,
    0
  )
);
...
describe('surveyPolisDemoResultsData', () => {
  it('normalizes the canonical Polis demo comments and votes into question-results cache data', () => {
    ...
    expect(data.questions[firstQuestionId]).toEqual(expect.objectContaining({
      id: firstQuestionId,
    ...
    expect(firstQuestionResponses[0]).toEqual(expect.objectContaining({
      type: 'binary',
  ...
  it('stamps an explicit session slug when callers need a non-default demo bucket', () => {
    ...
    expect(data.questions['0xabc']).toEqual(expect.objectContaining({
      id: '0xabc',
      prompt: 'Fixture question',
      sessionSlug: 'demo-source',
      sessionSlugExplicit: true,
    }));
    expect(data.questionResponses['0xabc']['0x123']).toEqual(expect.objectContaining({
  ...
  it('normalizes canonical demo votes into Polis report aggregator rows', () => {
    ...
    expect(firstRows[0]).toEqual(expect.objectContaining({
