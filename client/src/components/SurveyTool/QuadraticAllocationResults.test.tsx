...
it('does not count a comment-only response as encrypted or invalid', () => {
  ...
  expect(summarizeQuadraticAllocations(responses, question)).toMatchObject({ totalResponders: 0, excludedResponses: 0 });
...
it('still counts zero allocations and excludes encrypted or malformed allocations', () => {
  expect(summarizeQuadraticAllocations([
    { answer: { value: [0, 0] } }, { answer: { encrypted: true } }, { answer: { value: [10, 0] } },
  ], question)).toMatchObject({ totalResponders: 1, excludedResponses: 2 });
});
