import { buildSurveyResultsAlertMessagePatch } from './surveyResultsHelpers';
import type { SurveyResultsHtmlReportReadinessPlan } from './surveyResultsHtmlReportReadiness';
import type { SurveyResultsHtmlReportDownloadStatePatch } from './surveyResultsHtmlReportStatePatches';
...
export type SurveyResultsHtmlReportDownloadAttemptPlan =
  ...
  | {
      blockedReason:
        'not-authorized' | 'no-exportable-sections' | 'analysis-generating' | 'unavailable-selected-sections';
...
export const buildSurveyResultsHtmlReportDownloadAttemptPlan = ({
  ...
  readinessPlan: Pick<SurveyResultsHtmlReportReadinessPlan, 'hasExportableSections' | 'hasUnavailableSelectedSections'>;
}): SurveyResultsHtmlReportDownloadAttemptPlan => {
  if (!isAuthorized) {
    return {
      blockedReason: 'not-authorized',
      statePatch: buildSurveyResultsAlertMessagePatch(
  ...
  if (analysisGenerating) {
    return {
      ...
      statePatch: buildSurveyResultsAlertMessagePatch(
        'Wait for analysis generation to finish before downloading the report.',
      ),
      status: 'blocked',
    };
  }
