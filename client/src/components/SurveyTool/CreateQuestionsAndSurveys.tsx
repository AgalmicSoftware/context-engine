...
type CreateSurveyGateSessionConfig = CreateSurveySessionConfigLike & {
  encryption?: {
    gates?: unknown;
    [key: string]: unknown;
  };
...
class CreateQuestionsAndSurveys extends Component<CreateQuestionsAndSurveysProps, CreateQuestionsAndSurveysState> {
  ...
  resolveGateOptions = (
    ...
    const cfg = (cfgIn && typeof cfgIn === 'object') ? cfgIn as CreateSurveyGateSessionConfig : {};
    ...
    if (!relevantGates.length) {
      const resources = isPlainObject(cfg?.sponsored?.resources)
        ? cfg.sponsored.resources as Record<string, UnknownRecord | undefined>
        : {};
  ...
  render() {
    ...
    const applyDefaultSelectedGateIds = (value: unknown) => {
      ...
      return normalized.length ? normalized : (defaultGateId ? [defaultGateId] : []);
    };
          ...
          <div className={styles.surveySubmissionConfirmation} data-testid={E2E_TESTIDS.CREATE_SUCCESS}>
            ...
            {uploadedQuestions && uploadedQuestions.length > 0 && (
              <div className={styles.uploadedQuestionsList} data-testid={E2E_TESTIDS.CREATE_UPLOADED_QUESTIONS}>
                ...
                <ul>
                  {uploadedQuestionEntries.map(({ questionId, arweaveTxId }, index: number) => {
                    const idL = String(questionId).toLowerCase();
