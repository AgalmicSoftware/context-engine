import {
  bindSurveyResultsAnalysisGenerationPort,
...
describe('surveyResultsAnalysisGenerationPort', () => {
  it('calls AI with the existing analysis request options', async () => {
    ...
    await expect(port.generateSection({
      maxTokens: 1200,
  ...
  it('performs call-time AI lookup so module mocks keep intercepting', async () => {
    ...
    expect(firstCallAI).toHaveBeenCalledWith('first prompt', expect.objectContaining({
      maxTokens: 1,
      sessionSlug: 'alpha',
    }));
    expect(secondCallAI).toHaveBeenCalledWith('second prompt', expect.objectContaining({
      maxTokens: 2,
      sessionSlug: 'beta',
    }));
  });
