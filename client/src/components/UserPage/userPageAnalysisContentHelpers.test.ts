...
it('keeps quadratic option order and budgets in profile analysis and excludes malformed allocations', () => {
  ...
  expect(
    buildUserPageAnalysisQuestions({
      questionResponseInfo: [question],
      detailedQuestionResponses: { q1: response },
    })[0],
  ).toMatchObject({ options: ['Parks', 'Transit'], voiceCredits: 25, answer: [3, -4] });
  expect(
    buildUserPageAnalysisSurveys({
      surveyResponseInfo: [{ id: 's1' }],
      detailedSurveyResponses: { s1: [{ questionData: question, responseData: response }] },
    })[0],
  ).toMatchObject({
    sample: [expect.objectContaining({ options: ['Parks', 'Transit'], voiceCredits: 25, answer: [3, -4] })],
  });
  expect(
    buildUserPageAnalysisQuestions({
      questionResponseInfo: [question],
      detailedQuestionResponses: { q1: { answer: { value: [4, -4] } } },
    }),
  ).toEqual([]);
  expect(buildUserPageAnalysisCreatedQuestions([question])[0]).toMatchObject({
