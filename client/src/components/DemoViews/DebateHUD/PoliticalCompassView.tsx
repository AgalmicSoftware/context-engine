...
const PoliticalCompass = ({ compass, compact = true }: PoliticalCompassProps) => {
  ...
  const getPointSlug = (pointName?: string) =>
    String(pointName || 'point')
      ...
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-+|-+$/g, '');
  const getPointTestId = (pointName?: string) => `ce-political-compass-point-${getPointSlug(pointName)}`;
  ...
  const handlePointMouseEnter = (event: React.MouseEvent<SVGGElement>, point: CompassPoint, pointComment: string) => {
  ...
  const splitEdgeLabel = (label: string) => {
    ...
    const splitAt = spaces.reduce(
      (closest, index) => (Math.abs(index - midpoint) < Math.abs(closest - midpoint) ? index : closest),
      spaces[0],
    );

    return [normalized.slice(0, splitAt).trim(), normalized.slice(splitAt + 1).trim()].filter(Boolean);
  };
  ...
  const compassContent = (
    <>
          ...
          return (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                <div>
                  {profileHref ? (
                    <a href={profileHref} style={selectedPointTitleStyle} target="_blank" rel="noopener noreferrer">
                      {selectedPoint}
                    </a>
                  ) : (
              ...
              {pointComment ? (
                ...
                >
                  <div style={{ marginBottom: 8 }}>&quot;{pointComment}&quot;</div>
                  <div
                    style={{
                      fontSize: 11,
                      fontStyle: 'normal',
                  ...
                  >
                    {profileHref ? (
                      <a
                        href={profileHref}
                        style={selectedPointCitationStyle}
                        target="_blank"
                ...
                <>
                      ...
                      {profile.keyClaims.slice(0, 3).map((c, i) => (
                        <div
                          ...
                          style={{
                            ...
                            padding: '4px 0',
                            borderBottom:
                              i < Math.min(profile.keyClaims?.length || 0, 3) - 1
                                ? `1px solid ${T.borderLight}`
                                : 'none',
                            lineHeight: 1.5,
                          }}
                        >
                          <span style={{ color: (sp && sp.color) || T.accent, marginRight: 6, fontWeight: 700 }}>
                            →
                          </span>
                          {c}
                        </div>
                  ...
                  {quotes.length > 0 && (
                    <div
                      style={{
                        marginBottom: profile && profile.policyPositions && profile.policyPositions.length ? 12 : 0,
                      }}
                    >
                      <div
                        style={{
                      ...
                      {quotes.map((q, i) => (
                        ...
                        >
                          ...
                          {q.source && !q.url && <span style={{ fontSize: 10, color: T.textMuted }}>— {q.source}</span>}
                        </div>
                      ))}
                    </div>
                  )}
      ...
      <div ref={wrapperRef} data-testid="ce-political-compass-chart-wrapper" style={{ position: 'relative' }}>
        <svg
        ...
        >
          ...
          <text x={cx} y={padY + cH + 28} textAnchor="middle" style={edgeLabelStyle}>
        ...
        {avatarPointLayouts.map(({ point, px, py, isActive, avatarInfo, avatarRadius }) => (
          <img
  ...
  return isFullscreen ? (
    ...
    >
      ...
      <div style={{ width: '100%', maxWidth: 'min(90vh, 900px)' }}>{compassContent}</div>
    </div>
  ) : (
    <div
      style={{
...
const StandalonePoliticalCompass = ({
  compass,
  theme = darkTheme,
  compact = false,
}: StandalonePoliticalCompassProps) => (
