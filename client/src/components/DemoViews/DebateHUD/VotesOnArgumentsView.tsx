...
const CastYourVote = ({ debate, userVotes, setUserVotes }: CastYourVoteProps) => {
  ...
  return (
    <div style={{ maxWidth: 900 }}>
      <div style={{
        padding: "24px", background: T.surface, borderRadius: T.radius,
        border: `1px solid ${T.border}`, marginBottom: 16, boxShadow: T.shadow, textAlign: "center"}}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, justifyContent: "center", marginBottom: 12 }}>
          ...
          {totalUserVotes > 0 && (
            <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 10, background: soften(T.accent, 0.1), color: T.accent, fontWeight: 600 }}>
          ...
          {([
            ...
            return (
              <button key={side} onClick={() => castVote(side)} style={{
                flex: 1, maxWidth: 300, padding: "20px",
              ...
              onMouseEnter={e => { if (!selected) e.currentTarget.style.borderColor = data.color; }}
              onMouseLeave={e => { if (!selected) e.currentTarget.style.borderColor = T.border; }}
              >
                <div style={{ fontSize: 36, marginBottom: 8 }}>{data.figure.name === debate.sideA.figure.name ? "🟦" : "🟥"}</div>
                <div style={{ fontSize: 16, fontWeight: 700, color: data.color, marginBottom: 4 }}>{data.figure.name}</div>
                <div style={{ fontSize: 12, color: T.textMuted, lineHeight: 1.4 }}>{data.figure.voice}</div>
                {selected && <div style={{ marginTop: 10, fontSize: 12, fontWeight: 600, color: data.color }}>✓ Your vote</div>}
              </button>
            );
          })}
        </div>
      ...
      {userVote && (
        <div style={{
          padding: "20px 24px", background: T.surface, borderRadius: T.radius,
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 12 }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ height: 12, borderRadius: 6, overflow: "hidden", display: "flex", background: T.borderLight }}>
                <div style={{ width: `${Math.round((tally.a / tally.total) * 100)}%`, background: debate.sideA.color }} />
                <div style={{ width: `${Math.round((tally.b / tally.total) * 100)}%`, background: debate.sideB.color }} />
              </div>
            </div>
          </div>
          <p style={{ fontSize: 13, color: T.text, margin: 0 }}>
            You voted for <strong style={{ color: userVote === "A" ? debate.sideA.color : debate.sideB.color }}>
              ...
              {audienceRoster
                ...
                .map((m: AudienceMember) => (
                  <span key={m.name} style={{
                    ...
                    color: userVote === "A" ? debate.sideA.color : debate.sideB.color}}>{m.icon} {m.name}</span>
                ))
              }
            </div>
          </div>
          ...
          {userCompassPos && debate.compass && (
            <div style={{ marginTop: 16 }}>
              ...
              <div style={{ position: "relative", width: "100%", maxWidth: 300, aspectRatio: "1", background: T.bg, borderRadius: T.radiusSm, border: `1px solid ${T.border}`, overflow: "hidden" }}>
                <div style={{ position: "absolute", top: 0, left: 0, width: "50%", height: "50%", background: soften("#e53935", 0.08) }} />
                <div style={{ position: "absolute", top: 0, right: 0, width: "50%", height: "50%", background: soften("#1e88e5", 0.08) }} />
                <div style={{ position: "absolute", bottom: 0, left: 0, width: "50%", height: "50%", background: soften("#43a047", 0.08) }} />
                ...
                <div style={{
                  position: "absolute", left: `${userCompassPos.x * 100}%`, top: `${(1 - userCompassPos.y) * 100}%`,
                  width: 14, height: 14, borderRadius: "50%",
                  background: userVote === "A" ? debate.sideA.color : debate.sideB.color,
                  border: "2px solid #fff", boxShadow: "0 0 8px rgba(0,0,0,0.3)",
                  transform: "translate(-50%, -50%)", zIndex: 5}} />
                <div style={{
                  position: "absolute", left: `${userCompassPos.x * 100}%`, top: `${(1 - userCompassPos.y) * 100 + 5}%`,
                  transform: "translateX(-50%)", fontSize: 9, fontWeight: 700,
                  color: userVote === "A" ? debate.sideA.color : debate.sideB.color,
                  textShadow: `0 0 4px ${T.surface}`, zIndex: 5}}>YOU</div>
              </div>
            </div>
          )}
        </div>
            ...
            {debateData.map((d: Debate) => {
              ...
              return (
                <div key={d.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, padding: "4px 0" }}>
                  ...
                  {v ? (
                    <span style={{ padding: "2px 8px", borderRadius: 8, fontSize: 10, fontWeight: 600, background: soften(v === "A" ? d.sideA.color : d.sideB.color, 0.15), color: v === "A" ? d.sideA.color : d.sideB.color }}>
