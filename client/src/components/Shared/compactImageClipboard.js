...
const SUPPORTED_IMAGE_MIME_TO_EXT = Object.freeze({
  'image/png': 'png',
  'image/jpeg': 'jpeg',
  'image/jpg': 'jpg',
  'image/gif': 'gif',
...
const isSupportedArweaveReference = (value) => {
...
const isSupportedRelativeAssetPath = (value) => /^assets\/\S+$/i.test(toText(value).trim());

const isSupportedClipboardText = (value) => {
  const raw = toText(value).trim();
  if (!raw) return false;
...
const buildClipboardFile = ({ blob, imageType, fileNamePrefix }) => {
...
export const readCompactImageClipboard = async ({ fileNamePrefix = 'clipboard-image' } = {}) => {
