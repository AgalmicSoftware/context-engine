...
describe('sessionWizardDeployErrors', () => {
  it('builds the deploy-helper CORS message with origin and helper details', () => {
    expect(buildSessionWizardDeployHelperCorsMessage({
  ...
  it('summarizes workers.dev account and script status details', () => {
    expect(buildSessionWizardDeployHelperWorkersDevStatusMessage({
    ...
    expect(withSessionWizardDeployHelperWorkersDevStatus('Worker deployed.', {
  ...
  it('formats bundle diagnostics as a compact summary', () => {
    expect(formatSessionWizardDeployBundleDiagnostics({
      source: 'remote-url',
      length: 216,
      ...
      'source=remote-url len=216 sha256=0123456789abcdef export=1 default=0 namedDefault=0 fetch=0 swFetch=0'
    );
  });
