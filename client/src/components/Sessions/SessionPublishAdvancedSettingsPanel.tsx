...
const SessionPublishAdvancedSettingsPanel = ({
  ...
  workerUrlSource,
}: SessionPublishAdvancedSettingsPanelProps): React.ReactElement | null => {
  const { showArweaveMetadataControls, showGasOverrideControls } = publishSettingsCapabilities;
  if (!showArweaveMetadataControls && !showGasOverrideControls) return null;

  return (
    <>
      {showArweaveMetadataControls ? (
        <>
          ...
          <FormGroup className={styles.fieldGroup}>
            <Label className={styles.fieldLabelRow}>
              ...
              {renderInfoTooltip({
                id: 'gw-tip-gas-price',
                content:
                  'Optional. Forces a legacy gas price (type 0). Some wallets may ignore this on EIP-1559 networks.',
                placement: 'right',
