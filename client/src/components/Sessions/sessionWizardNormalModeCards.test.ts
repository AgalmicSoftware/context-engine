...
describe('sessionWizardNormalModeCards', () => {
  describe('buildNormalModeCards', () => {
    it('includes or omits the worker step and assigns visible step numbers', () => {
      expect(buildNormalModeCards({
        ...baseCardsInput,
        showNormalModeWorkerStep: true,
      }).map(({ key, stepNumber }) => [key, stepNumber])).toEqual([
        ['metadata', 1],
        ['encryption', 2],
        ['worker', 3],
        ['publish', 4],
  ...
  describe('buildNormalModePublishSummary', () => {
    it('shows pending SBT draft counts', () => {
      expect(buildNormalModePublishSummary({
        ...basePublishSummaryInput,
        pendingDraftCount: 1,
      }).find((item) => item.label === 'Pending SBTs')?.value).toBe('1 draft ready');

      expect(buildNormalModePublishSummary({
        ...basePublishSummaryInput,
        pendingDraftCount: 3,
      }).find((item) => item.label === 'Pending SBTs')?.value).toBe('3 drafts ready');

      expect(buildNormalModePublishSummary({
        ...basePublishSummaryInput,
        pendingDraftCount: 0,
      }).find((item) => item.label === 'Pending SBTs')?.value).toBe('None');
    });
    ...
    it('labels worker states in publish summary order', () => {
      ...
      }).find((item) => item.label === 'Worker')?.value).toBe('Sponsored auto-deploy on Publish');

      expect(buildNormalModePublishSummary({
        ...basePublishSummaryInput,
        shouldUseSponsoredAutoDeployFlow: true,
      }).find((item) => item.label === 'Worker')?.value).toBe('Sponsored auto-deploy waiting for the hosted bundle URL');

      expect(buildNormalModePublishSummary({
        ...basePublishSummaryInput,
        normalModeRequiresCustomWorker: true,
        resolvedWorkerBaseUrl: '',
      }).find((item) => item.label === 'Worker')?.value).toBe('Bring your own worker');

      expect(buildNormalModePublishSummary({
        ...basePublishSummaryInput,
        normalModeRequiresCustomWorker: true,
      }).find((item) => item.label === 'Worker')?.value).toBe('Custom worker ready');

      expect(buildNormalModePublishSummary({
        ...basePublishSummaryInput,
        workerMode: 'default',
      }).find((item) => item.label === 'Worker')?.value).toBe('Shared hosted worker');

      expect(buildNormalModePublishSummary({
        ...basePublishSummaryInput,
        workerMode: 'custom',
        deployVerifiedInUi: true,
      }).find((item) => item.label === 'Worker')?.value).toBe('Custom worker deployed');

      expect(buildNormalModePublishSummary({
        ...basePublishSummaryInput,
        workerMode: 'custom',
        deployVerifiedInUi: false,
      }).find((item) => item.label === 'Worker')?.value).toBe('Custom worker setup');
    });
  });
});
