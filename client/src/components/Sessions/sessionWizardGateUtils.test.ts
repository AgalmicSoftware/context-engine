...
describe('sessionWizardGateUtils', () => {
  it('builds default gate state and resource mappings', () => {
    expect(buildDefaultGateState(84532)).toEqual(expect.objectContaining({
      default: {
        sbts: [],
        mode: 'all',
        chainId: 84532,
    ...
    expect(buildResourceGateMap([{ id: 'gate-3' }], 'gate-1')).toEqual(expect.objectContaining({
      default: 'gate-1',
      ai: 'gate-1',
      lit: 'gate-1',
    }));
  ...
  it('resolves on-chain keys, secret paths, lists, and gate numbering', () => {
    ...
    expect(buildEncryptionGate(2)).toEqual(expect.objectContaining({
      id: 'gate-3',
  ...
  it('builds an empty sponsored context and resolves selector source config fallbacks', () => {
    ...
    expect(resolveSessionWizardSelectorSourceConfig({
