...
describe('sbtSessionUniverse', () => {
  ...
  it('resolves demo entry keys through canonical demo-session aliases when cfg.slug is missing', () => {
    expect(resolveSessionUniverseEntrySlug(
      ['legacyReading', { sessionName: 'Reading Group' }],
      { demoSessionMap }
    )).toBe('reading-group');

    expect(resolveSessionUniverseEntrySlug(
      ['customUniverseSession', { sessionName: 'Custom Universe Session' }],
      { demoSessionMap }
  ...
  it('dedupes registry and demo universe entries by canonical slug', () => {
    const merged = mergeSessionUniverseEntriesBySlug([
      [['custom-universe-session', { sessionName: 'Registry Custom Session' }]],
      [['customUniverseSession', {}]],
    ], { demoSessionMap });
