...
describe('CreateSBTGroup deterministic deploy payloads', () => {
  ...
  it('exports pre-signed independent slots instead of the reusable limited-group secret', async () => {
    ...
    instance.state = {
      ...instance.state,
      sbtName: 'Invite Group',
      tokenURI: 'ar://metadata',
      groupPassword: 'creator-only-secret',
      create2Salt: 'invite-salt',
      sbtDistribution: {
    ...
    const generate = jest.spyOn(contractScripts, 'generateInvitePayloads').mockResolvedValue([
      { nonce: '1', inviteCode: 'signed-slot-one' },
      { nonce: '2', inviteCode: 'signed-slot-two' },
    ]);
    await instance.mintSBT();
    expect(generate).toHaveBeenCalledWith(
      expect.objectContaining({
        password: 'creator-only-secret',
        sbtAddress: address,
        chainId: 84532,
        nonces: ['1', '2'],
      }),
