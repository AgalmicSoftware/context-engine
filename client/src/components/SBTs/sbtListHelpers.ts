...
export const collectSbtTagValues = (...candidates: unknown[]): string[] => {
  ...
  const visit = (candidate: unknown): void => {
    ...
    if (isSbtListHelperRecord(candidate)) {
      const objectText = [
        candidate.label,
        candidate.name,
        candidate.value,
...
export const getSbtCardDetails = (sbt: unknown): SbtCardDetails => {
  const record = isSbtListHelperRecord(sbt) ? sbt as SbtListHelperItem : {};
  const sbtInfo = isSbtListHelperRecord(record.sbtInfo) ? record.sbtInfo : {};
...
export const buildSbtListDisplayCardModel = <T extends SbtListHelperItem>({
  ...
  const sbtInfoRecord = isSbtListHelperRecord(sbtInfo) ? sbtInfo : {};
  const sessionSlug = String(resolveSbtSessionSlug(sbt) || '');
  const sbtAddressLower = sbtAddress.toLowerCase();
