import type { HexString, PasskeyDerivedWalletRecord, PasskeyWalletConfig } from '../types.js';
...
const isValidPrivateKeyHex = (hex: string): boolean =>
  /^[0-9a-f]{64}$/.test(hex) && hex !== '0'.repeat(64) && hex < SECP256K1_ORDER_HEX;
...
export const getPasskeyDerivedPrfSalt = async (config: PasskeyWalletConfig): Promise<Uint8Array> => {
  ...
  const material = textEncoder.encode(`context-engine:passkey-derived-prf-salt:v1:${config.rpId}:${namespace(config)}`);
...
export const deriveEoaPrivateKeyFromPrf = async ({
  ...
  const salt = textEncoder.encode(`context-engine:passkey-derived-eoa-salt:v1:${config.rpId}:${namespace(config)}`);
    ...
    const bits = await crypto.subtle.deriveBits(
      {
        name: 'HKDF',
        hash: 'SHA-256',
        salt: bufferSourceToWebCryptoBufferSource(salt),
        info: bufferSourceToWebCryptoBufferSource(
          textEncoder.encode(`context-engine:passkey-derived-eoa:v1:${config.rpId}:${namespace(config)}:${counter}`),
        ),
      },
      baseKey,
      256,
