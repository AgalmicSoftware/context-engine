...
describe('litChipotlePolicy helpers', () => {
  it('normalizes supported v2 policy inputs without mutating caller-owned objects', () => {
    ...
    expect(normalizeChipotleSbtAddresses([SBT_B, SBT_A, SBT_B])).toEqual([
      SBT_A,
      SBT_B,
    ]);
  });
  ...
  it('binds wrapped CEK payloads to the canonical policy fingerprint', () => {
    ...
    expect(stableChipotleStringify({
      b: 2,
      a: { d: 4, c: 3 },
    })).toBe('{"a":{"c":3,"d":4},"b":2}');
  });
  ...
  it('rejects legacy and bare wrapped-key formats', () => {
    ...
    expect(() => parseLitChipotleWrappedPlaintext({ ...wrapped, v: 1 }))
      .toThrow('Lit Chipotle legacy wrapped keys are not supported.');
    expect(() => parseLitChipotleWrappedPlaintext({ cekHex: CEK_HEX }))
      .toThrow('Lit Chipotle legacy wrapped keys are not supported.');
    expect(() => parseLitChipotleWrappedPlaintext(null))
      .toThrow('Lit Chipotle legacy wrapped keys are not supported.');
    expect(() => parseLitChipotleWrappedPlaintext(CEK_HEX))
      .toThrow('Lit Chipotle wrapped key is not valid v2 JSON.');
  });
  ...
  it('rejects malformed policies, CEKs, and fingerprint mismatches', () => {
    ...
    expect(() => buildLitChipotlePolicy(makePolicyInput({ chainId: 0 })))
      .toThrow('Lit Chipotle policy requires a chain ID.');
    expect(() => buildLitChipotlePolicy(makePolicyInput({ sbtAddresses: [] })))
      .toThrow('Lit Chipotle policy requires at least one SBT address.');
    expect(() => buildLitChipotlePolicy(makePolicyInput({ sbtAddresses: ['not-an-address'] })))
      .toThrow('Lit Chipotle policy contains an invalid SBT address.');
    expect(() => buildLitChipotlePolicy(makePolicyInput({ litActionCid: '' })))
      .toThrow('Lit Chipotle policy requires a Lit Action CID.');
    expect(() => buildLitChipotlePolicy(makePolicyInput({ litPkpId: '' })))
      .toThrow('Lit Chipotle policy requires a Lit PKP ID.');
    expect(() => normalizeChipotleCekHex('0x1234'))
      .toThrow('Lit Chipotle CEK must be a 32-byte hex string.');
    expect(() => parseLitChipotleWrappedPlaintext({
      ...wrapped,
