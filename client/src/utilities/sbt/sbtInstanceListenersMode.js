...
export const normalizeSbtInstanceListenersMode = (raw) => {
  const val = toStr(raw).trim().toLowerCase();
  if (val === 'auto' || val === 'on' || val === 'off') return val;
  return 'auto';
};

export const readSbtInstanceListenersMode = () => {
  // Precedence:
  ...
  try {
    if (typeof globalThis !== 'undefined' && typeof globalThis[GLOBAL_KEY] !== 'undefined') {
      return normalizeSbtInstanceListenersMode(globalThis[GLOBAL_KEY]);
    }
  } catch (e) { log.warn('sbtInstanceListenersMode: fallback', e); }
...
export const writeSbtInstanceListenersMode = (modeIn) => {
  const mode = normalizeSbtInstanceListenersMode(modeIn);

  try {
    if (typeof globalThis !== 'undefined') globalThis[GLOBAL_KEY] = mode;
  } catch (e) { log.warn('sbtInstanceListenersMode: fallback', e); }
