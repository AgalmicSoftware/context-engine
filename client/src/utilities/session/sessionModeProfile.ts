...
export type SessionModeExportScope =
  | 'admin_raw'
  | 'all_session'
  | 'selected_surfaces'
...
export const isSessionModeProfileTelegramFirst = (metadata: unknown): boolean => {
  ...
  const profile = isRecord(config.sessionModeProfile)
    ...
    : (isRecord(metadata) && isRecord(metadata.authority) ? metadata : null);
...
const resourceStagesForBackend = (
  ...
  return SESSION_MODE_RESOURCE_KEYS.reduce<Record<SessionModeResourceKey, string>>((acc, key) => {
    const stage = resources?.[key]?.stage;
    acc[key] = stage === 'active' || stage === 'staged'
      ? stage
      : (key === 'docsContext' ? 'active' : defaultStage);
    return acc;
...
const hasParticipantGate = (profile: SessionModeProfile): boolean => (
  ...
  profile.authorization.mechanisms.some((mechanism) => (
...
export const cloneSessionModePreset = (
...
export const validateSessionModeProfile = (
  ...
  const mechanisms = Array.isArray(profile.authorization?.mechanisms)
...
export const compileSessionModeProfile = (
  profile: SessionModeProfile
  ...
  return {
    ...
    exportSurfaceFilter: Array.isArray(profile.export.surfaceFilter)
      ? [...profile.export.surfaceFilter]
      : [],
  };
};
...
export const profileFromLegacyConfig = (sessionConfig: unknown): SessionModeProfile => {
  ...
  const storageProfile = isRecord(config.storageProfile)
    ...
    : (isRecord(config.sessionStorageProfile) ? config.sessionStorageProfile : {});
  const normalizedBackend = normalizeStorageBackend(
  ...
  profile.evm.registryChainId = normalizeRegistryChainId(
    ...
    (isRecord(config.evm) ? config.evm.registryChainId : null)
  );
