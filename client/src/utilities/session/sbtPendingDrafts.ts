...
const normalizePendingSbtContractEntry = (value: unknown): PendingSbtContractEntry | null => {
  ...
  const address = toStr(source.address || source.contractAddress || source.addr || source.target || '').trim();
  const chainId = Number(source.chainId || source.chainID || source.networkChainId || source.chain || 0) || null;
...
export const clonePendingSbtDeployContracts = (contractsIn: unknown = {}): PendingSbtContractsMap => {
...
export const buildPendingSbtDeploySessionConfig = ({
  ...
  contracts: PendingSbtContractsMap;
} | null => {
  const source = isObj(sessionConfig) ? sessionConfig : {};

  const slug = toStr(source.slug || source.sessionSlug || sessionSlug).trim();
  const resolvedNetworkChainId = Number(source.networkChainId || source.chainId || networkChainId || 0) || null;
  const resolvedContracts = clonePendingSbtDeployContracts(isObj(contracts) ? contracts : source.contracts);
