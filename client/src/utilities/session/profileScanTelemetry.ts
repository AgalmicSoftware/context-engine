...
export function emitProfileScanTelemetry(
  ...
  try {
    ...
    const runtimeGlobals = globalThis as RuntimeGlobals;
    const bucket = Array.isArray(runtimeGlobals[key]) ? (runtimeGlobals[key] as Array<Record<string, unknown>>) : [];
    bucket.push(entry);
...
export function emitProfileScanColdDiag(this: ProfileScanTelemetryContext, event: string, payload: unknown = {}): void {
