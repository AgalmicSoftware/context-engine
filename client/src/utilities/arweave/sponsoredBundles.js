...
export const generateSponsoredBundleSecret = (byteLength = SPONSORED_BUNDLE_SECRET_BYTES) => {
...
export const normalizeSponsoredBundlePayload = (raw) => {
...
export const normalizeSparseSponsoredBundlePayload = (raw) => {
  const payload = isObj(raw) ? raw : {};
  const next = {};
  SPONSORED_BUNDLE_SUPPORTED_FIELDS.forEach((key) => {
...
export const hasSponsoredBundleFields = (bundle = {}) => (
...
export const buildSponsoredBundleEnvelope = ({ encryptedData } = {}) => {
...
export const parseSponsoredBundleEnvelope = (rawEnvelope) => {
...
export const isSponsoredBundleExpired = (bundle, nowMs = Date.now()) => {
