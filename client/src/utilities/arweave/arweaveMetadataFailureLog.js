...
const ARWEAVE_METADATA_FAILURE_LOG_MEMO = new Map();
...
const isCallExceptionError = (error) => {
  const queue = [error];
  const seen = new Set();
  while (queue.length > 0) {
    ...
    const code = String(current?.code ?? current?.error?.code ?? '').toUpperCase();
    const msg = String(
      current?.message ||
      current?.reason ||
    ...
    const requestBody = String(current?.requestBody || '').toLowerCase();
...
const buildArweaveMetadataFailureLogKey = ({ scope = '', meta = {} } = {}) => {
...
const shouldEmitArweaveMetadataFailureWarn = ({ scope = '', meta = {} } = {}) => {
...
const logArweaveMetadataFetchFailure = ({ scope = 'metadata', error = null } = {}) => {
  ...
  if (isTerminalArweaveFailureState(state)) {
    ...
    contractsLog.warn(
      ...
      failureMeta.message || error?.message || error
    );
    return;
  }
