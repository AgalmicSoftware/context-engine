...
export const isArweaveGatewayHost = (hostRaw) => {
...
export const readArweaveTxIdFromPathSegments = (segments = []) => {
...
export const normalizeGatewayBase = (value) => {
...
export const normalizeGatewayList = (rawList = []) => {
  const out = [];
  const seen = new Set();
...
export const normalizeBoolish = (value, defaultValue = false) => {
...
export const getRuntimeArweaveGatewayOverride = () => {
  try {
    if (typeof globalThis === 'undefined') return '';
    return normalizeGatewayBase(globalThis.CE_ARWEAVE_GATEWAY_URL);
  } catch {
    return '';
  }
};

export const getRuntimeArweaveGatewayFallbacks = () => {
  try {
    if (typeof globalThis === 'undefined') return [];
    const raw = globalThis.CE_ARWEAVE_GATEWAYS;
...
export const isDirectToArIoEnabled = (opts = {}) => {
  if (typeof opts?.directToArIo === 'boolean') return opts.directToArIo;
  try {
    if (typeof globalThis !== 'undefined' && typeof globalThis.CE_ARWEAVE_DIRECT_TO_AR_IO !== 'undefined') {
      return normalizeBoolish(globalThis.CE_ARWEAVE_DIRECT_TO_AR_IO, !!CE_ARWEAVE_DIRECT_TO_AR_IO);
    }
  } catch (e) { void e; /* fallback to static config. */ }
  return !!CE_ARWEAVE_DIRECT_TO_AR_IO;
};

export const getRuntimeArIoGatewayOverride = () => {
  try {
    if (typeof globalThis === 'undefined') return '';
    return normalizeGatewayBase(globalThis.CE_ARWEAVE_AR_IO_URL);
  } catch {
    return '';
  }
};

export const getPreferredArIoGateway = (gatewayOverride = '') => (
...
export const getPreferredArweaveGateway = (gatewayOverride = '') => (
  normalizeGatewayBase(gatewayOverride) ||
...
export const getDefaultArweaveGateways = (opts = {}) => (
...
export const isArweaveTxId = (value) => ARWEAVE_TXID_RE.test(toStr(value).trim());
...
export const buildArweaveGatewayUrl = (value, options = {}) => (
