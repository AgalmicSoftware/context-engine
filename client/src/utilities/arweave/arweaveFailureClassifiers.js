...
export const isTerminalArweaveFailureState = (state) => (
...
export const normalizeArweaveFailureEntry = (rawEntry) => {
...
export const normalizeArweaveFailureMeta = (raw) => {
  const fromError = raw && typeof raw === 'object' ? raw : {};
  const nested = (fromError.arweaveFailure && typeof fromError.arweaveFailure === 'object')
...
export const computeArweaveFailureBackoffMs = (attempts) => {
...
export const classifyArweaveFailureState = ({ txId, prevEntry, error }) => {
  const now = Date.now();
...
export const buildHashUnavailableMetadataError = (message, meta = {}) => {
  const retryAtMs = Number(meta?.nextRetryAtMs || 0) > 0
    ? Number(meta.nextRetryAtMs)
    : (Date.now() + ARWEAVE_TX_FAILURE_TERMINAL_RECHECK_MS);
  const err = new Error(String(message || 'Metadata hash is unavailable'));
