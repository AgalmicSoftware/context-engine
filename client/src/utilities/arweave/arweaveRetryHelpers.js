...
const pickNewerFailureEntry = (a, b) => {
...
const mergeByKey = (localMap, freshMap, chooser) => {
  const local = (localMap && typeof localMap === 'object') ? localMap : {};
  const fresh = (freshMap && typeof freshMap === 'object') ? freshMap : {};
  const out = {};
...
const normalizePendingMetadataKey = (key) => String(key || '').trim().toLowerCase();
...
export const prunePendingMetadataEntries = ({
  ...
  const pending = (pendingEntries && typeof pendingEntries === 'object') ? pendingEntries : {};
  const hydrated = (hydratedEntries && typeof hydratedEntries === 'object') ? hydratedEntries : {};
  const nextPending = {};
  const removedKeys = [];
...
export const ensureQuestionArweaveCacheBranches = (networkNode) => {
...
export const mergeQuestionArweaveCacheBranches = (localNode, freshNode) => {
