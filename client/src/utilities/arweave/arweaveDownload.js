...
export function createArweaveDownloadOps({
  ...
  const recordTerminalArweaveInvalidFailure = async ({ groupKeyOrCfg, txId, message, cause = null }) => {
  ...
  const downloadArweaveTextForGroup = async ({ txId, groupKeyOrCfg, arweaveOpts = {} } = {}) => {
    const normalizedTxId = String(txId || '').trim();
      ...
      task: async () => {
        ...
        try {
          ...
          contractsLog.warn('[arweave-cache] tx-fetch-success-cache-persist-fail', {
            txId: normalizedTxId,
            error: cachePersistError?.message || cachePersistError,
          });
        }
        ...
        try {
          ...
          contractsLog.warn('[arweave-cache] tx-fetch-success-failure-clear-fail', {
            txId: normalizedTxId,
            error: failureClearError?.message || failureClearError,
          });
        }
