...
const HISTORICAL_FIGURE_PHOTO_BY_USERNAME = Object.freeze({
...
const normalizeHistoricalFigureName = (value = '') => transliterate(value)
  .normalize('NFD')
  .replace(/[\u0300-\u036f]/g, '')
  .replace(/&/g, 'and')
  .replace(/[^a-zA-Z0-9]+/g, '')
...
const HISTORICAL_FIGURE_USERNAME_BY_NORMALIZED_NAME = Object.freeze(
  HISTORICAL_FIGURE_USERNAMES.reduce((acc, username) => {
    const entry = HISTORICAL_FIGURE_PHOTO_BY_USERNAME[username] || {};
    [
      username,
      entry?.name,
    ]
...
const buildHistoricalFigureBlockieSeed = (usernameIn = '') => {
  const username = String(usernameIn || '').trim().toLowerCase();
  return username ? `historical:${username}` : 'historical-figure';
};

export const getHistoricalFigureBlockie = (usernameIn = '', options = {}) => {
...
export const hasHistoricalFigureAvatar = (usernameIn = '') => (
  HISTORICAL_FIGURE_USERNAME_SET.has(String(usernameIn || '').trim())
);

export const getHistoricalFigureAvatar = (usernameIn = '') => {
  const username = String(usernameIn || '').trim();
  if (!username) return '';
  return buildPublicAssetPath(HISTORICAL_FIGURE_PHOTO_BY_USERNAME[username]?.src || '');
};

export const getHistoricalFigureAvatarByName = (nameIn = '') => {
...
export const getHistoricalFigureAvatarOrBlockie = (usernameIn = '', options = {}) => {
  const username = String(usernameIn || '').trim();
...
export const getHistoricalFigureAvatarUsernames = () => HISTORICAL_FIGURE_USERNAMES.slice();
