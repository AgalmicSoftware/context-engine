...
const isVerboseRpcErrorsEnabled = () => {
  try {
    if (typeof window === 'undefined') return false;
    return window.CE_RPC_VERBOSE_ERRORS === true;
  } catch (_) {
    return false;
  }
};

export function truncateRpcString(value, maxLen = RPC_ERROR_TREE_MAX_STRING) {
...
export function summarizeRpcError(err, depth = RPC_ERROR_TREE_MAX_DEPTH, seen = new Set()) {
  ...
  if (Array.isArray(err?.errors)) {
    summary.errors = err.errors.slice(0, RPC_ERROR_TREE_MAX_ARRAY)
      .map((e) => summarizeRpcError(e, depth - 1, seen));
  }
...
export function logVerboseRpcError(label, err, meta = {}) {
