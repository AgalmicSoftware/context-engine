...
export function createContractEventListenerMethods(deps) {
  ...
  const buildListenerKey = ({ address, chainId, provider, fallbackScope = 'default' }) => (
    [
  ...
  return {
    async listenForSBTEvents(providerName, handleNewEvent, groupKeyOrCfg = null) {
    ...
    removeSBTEventListener(providerName, groupKeyOrCfg = null) {
      const cfg = resolveSession(groupKeyOrCfg || '');
      const gAddrs = getSessionAddresses(cfg);
      const addr = gAddrs.sbtFactory?.address;
      const chId = resolveChainIdForContract(cfg, 'sbtFactory');
    ...
    async listenForSBTInstanceEvents(providerName, sbtAddresses = [], handler, groupKeyOrCfg = null) {
      const provider =
        (typeof getReadProviderForGroup === 'function' && getReadProviderForGroup(groupKeyOrCfg, { contractKey: 'sbtFactory' }));
      ...
      const input = Array.isArray(sbtAddresses) ? sbtAddresses : [sbtAddresses];
      ...
      const registry = (this.listenForSBTInstanceEvents._registry ||= new Map());
      const keyOf = (address) => [address.toLowerCase(), chainId, providerScope].join('||');
      ...
      for (const addr of uniqueAddresses) {
        ...
        try {
          ...
          const onActivity = (account, tokenId, burned, event) => {
            try {
              handler && handler({
                type: 'SBTActivity',
                address: addr,
    ...
    async removeSBTInstanceEventsListener(providerName, sbtAddresses = [], groupKeyOrCfg = null) {
      ...
      const list = Array.isArray(sbtAddresses) ? sbtAddresses : [sbtAddresses];
    ...
    async listenForSurveyEvents(providerName, handleNewEvent, groupKeyOrCfg = null) {
      const cfg = resolveSession(groupKeyOrCfg || '');
      const gAddrs = getSessionAddresses(cfg);
    ...
    removeSurveyEventsListener(providerName, groupKeyOrCfg = null) {
      const cfg = resolveSession(groupKeyOrCfg || '');
      const gAddrs = getSessionAddresses(cfg);
      const addr = gAddrs.surveys?.address;
      const chId = resolveChainIdForContract(cfg, 'surveys');
