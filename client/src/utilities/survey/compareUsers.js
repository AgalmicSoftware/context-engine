...
export function getCompareSbtLabel(entry = null) {
  ...
  const displayName = info && typeof info === 'object'
    ? getSbtDisplayName(info)
    : '';
  return String(
    displayName ||
    entry?.name ||
...
export function getCompareSbtKey(entry = null) {
  ...
  const address = toLower(
    entry?.compareKey ||
    entry?.address ||
    entry?.sbtAddress ||
    entry?.sbtInfo?.sbtAddress ||
    ''
  );
...
function normalizeRatingSignedValue(valRaw) {
  ...
  const scales = [
    { min: 0, max: 1 }, { min: 1, max: 5 }, { min: 1, max: 7 }, { min: 0, max: 10 }, { min: 1, max: 10 },
  ];
...
export function opinionVennTriplet(users3) {
  ...
  const pairSets = encs.map(e => {
    const S = new Set();
    e.tokens.forEach(({sign}, tok) => { if (sign !== 0) S.add(`${tok}::${sign}`); });
    return S;
  });
  ...
  const ab = inter(A,B) - abc, ac = inter(A,C) - abc, bc = inter(B,C) - abc;
...
export function computeVennEvidence(users) {
  ...
  const pairSet = (enc) => {
    const S = new Set();
    enc.tokens.forEach(({ sign }, tok) => { if (sign !== 0) S.add(`${tok}::__${sign}`); });
    return S;
  };
  const pairSets = encs.map(pairSet);
  const inter = (S1, S2) => { const out = new Set(); S1.forEach(v => { if (S2.has(v)) out.add(v); }); return out; };
  const diff = (S1, S2) => { const out = new Set(); S1.forEach(v => { if (!S2.has(v)) out.add(v); }); return out; };
  const union = (S1, S2) => { const out = new Set(S1); S2.forEach(v => out.add(v)); return out; };

  // qid -> prompt lookup for compact labels
  const qMeta = new Map();
  (arr || []).forEach((u) => (u?.questions || []).forEach((q) => {
...
export function pcaLiteCompass(users = []) {
  ...
  const tokenSet = new Set(); encs.forEach(e => e.tokens.forEach((_, t) => tokenSet.add(t)));
  const tokens = Array.from(tokenSet);
  const U = safe.length, T = tokens.length;
  const axes = [
  ...
  for (let i=0;i<U;i++){
    ...
    for (let j=0;j<T;j++){ const cell = m.get(tokens[j]); X[i][j] = cell ? cell.sign*cell.weight : 0; }
  }
  const means = Array(T).fill(0);
  for (let j=0;j<T;j++){ let s=0; for (let i=0;i<U;i++) s+=X[i][j]; means[j]=s/U; for (let i=0;i<U;i++) X[i][j]-=means[j]; }
  let allZero = true;
  outer: for (let i=0;i<U;i++){ for (let j=0;j<T;j++){ if (X[i][j] !== 0) { allZero = false; break outer; } } }
  if (allZero) {
    const points = addrList.map((a,i)=>({ address:a, x: (U>1? clamp(-1 + (2*i)/(U-1), -1,1):0), y:0 }));
    return { axes, points, evidence:{x:[],y:[]} };
  }
  ...
  const Av = (v)=>{ const y=Array(U).fill(0); for (let i=0;i<U;i++){ let s=0; const row=X[i]; for (let j=0;j<T;j++) s+=row[j]*v[j]; y[i]=s; } return y; };
  const ATy = (y)=>{ const z=Array(T).fill(0); for (let j=0;j<T;j++){ let s=0; for (let i=0;i<U;i++) s+=X[i][j]*y[i]; z[j]=s; } return z; };
  const dot=(a,b)=>a.reduce((s,v,i)=>s+v*b[i],0), norm=(v)=>Math.hypot(...v), scale=(v,k)=>v.map(x=>x*k), sub=(a,b)=>a.map((x,i)=>x-b[i]);
  const powerIter = (q=null, iters=10)=>{ let v=Array(T).fill(0).map(()=>rand()-0.5); let vn=norm(v)||1; v=scale(v,1/vn);
    for (let k=0;k<iters;k++){ let w=ATy(Av(v)); if (q){ const proj=dot(w,q); w=sub(w, scale(q, proj)); } const wn=norm(w)||1; v=scale(w,1/wn); }
    return v;
  };
  const v1 = powerIter(null,10), v2 = powerIter(v1,10);
  const s1 = Av(v1), s2 = Av(v2);
  ...
  const Xmax = maxAbs(S1), Ymax = maxAbs(S2);
...
export function computeOverlapMatrix(users = [], topN = 20) {
  ...
  const qMeta = new Map();
  (users||[]).forEach(u => (u?.questions||[]).forEach(q => {
    const id = String(q?.id || q?.questionID || q?.questionId || '').toLowerCase();
    if (id && !qMeta.has(id)) qMeta.set(id, { prompt: String(q?.prompt||''), tags: Array.isArray(q?.tags) ? q.tags : [] });
  }));
  const labelForToken = (tok) => {
    const s = String(tok||''); const idx=s.indexOf('::'); const qid = idx===-1? s : s.slice(0,idx);
  ...
  if (haveSignal) {
    ...
    if (topTokens.length > 0) {
      ...
      const rows = encs.map(enc => columns.map(col => {
  ...
  const userSets = (users||[]).map(u => {
    const set = new Set(); (u?.sbts||[]).forEach(s => {
  ...
  const topNames = Array.from(nameCounts.entries()).sort((a,b)=>(b[1]-a[1])||(a[0]<b[0]?-1:1)).slice(0,N);
...
export function sanitizeCompass(bundle, addressesInOrder = []) {
  ...
  const axes = [
    { id:'x', label:String(axX?.label||'Axis 1'), description:String(axX?.description||'First principal direction of encoded opinions.') },
    { id:'y', label:String(axY?.label||'Axis 2'), description:String(axY?.description||'Second principal direction of encoded opinions.') },
  ];
  const ptsMap = new Map();
  (Array.isArray(bundle.points)?bundle.points:[]).forEach(p => {
    const a = String(p?.address||'').toLowerCase(); if (!a) return;
    const x = clamp(Number(p?.x??0), -1,1), y = clamp(Number(p?.y??0), -1,1);
    ptsMap.set(a, { address:a, x, y });
  });
  const list = Array.isArray(addressesInOrder) && addressesInOrder.length
    ? addressesInOrder.map(a => String(a||'').toLowerCase())
    : Array.from(ptsMap.keys());
  const points = list.map(a => ptsMap.get(a) || ({ address:a, x:0, y:0 }));
  const evidence = bundle?.evidence && typeof bundle.evidence === 'object'
    ? { x: Array.isArray(bundle.evidence.x)? bundle.evidence.x.slice(0,5).map(String):[], y: Array.isArray(bundle.evidence.y)? bundle.evidence.y.slice(0,5).map(String):[] }
    : { x:[], y:[] };
...
export function fallbackBullets(users = []) {
  try {
    ...
    if (common.length) {
      agreements.push(
        `Shared group memberships: ${common.slice(0,5).map((key) => labelByKey.get(key) || key).join(', ')}`
      );
    }
    ...
    const diffs = []; allPrompts.forEach(p => {
  ...
  const users = addrs.map((address) => {
    ...
    Object.keys(combinedSurveyResponses).forEach((sid) => {
      ...
      if (typeof respObj === 'string') {
        try { respObj = JSON.parse(respObj); } catch { respObj = null; }
      }
...
export function readBookmarksNormalized(rawJson) {
  const out=[]; try{
    ...
    const users=Array.isArray(parsed?.users)? parsed.users:[]; const seen=new Set();
    for (const u of users){
      let addrRaw=null, label=null;
      if (typeof u==='string'){ addrRaw=String(u||'').trim(); }
      else if (u && typeof u==='object'){ addrRaw=String(u.address||'').trim(); const nick=typeof u.nickname==='string'?u.nickname.trim():''; const uname=typeof u.username==='string'?u.username.trim():''; label = nick || uname || null; }
      if (!addrRaw) continue; const lower=addrRaw.toLowerCase(); if (seen.has(lower)) continue; seen.add(lower);
      out.push({ address: addrRaw, addressLower: lower, label: label || shortenPlain(addrRaw) });
    }
  }catch (e) { cacheLog.warn('compareUsers: fallback', e); }
  return out;
}
...
export function deriveUserLabels(users = [], bookmarks = []) {
  ...
  return (users||[]).map((u,i) => map.get(String(u?.address||'').toLowerCase()) || shortenPlain(u?.address||'') || `User ${i+1}`);
}
