...
describe('interview connection lifecycle', () => {
  ...
  it('retains legacy interviewer questions beside short replies and deduplicates completed outputs', async () => {
    const h = harness('realtime');
    const session = await h.start();
    const question = {
    ...
    expect(captured.turns.map((turn) => turn.role)).toEqual(['interviewer', 'responder', 'interviewer', 'responder']);
    expect(h.options.onTranscript).toHaveBeenLastCalledWith(captured.transcript, captured.turns);
  });
