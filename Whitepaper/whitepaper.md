...
Context Engine is an open-source toolkit enabling groups to create digital artifacts representing their preferences, beliefs, and points of agreement or disagreement over time. The approach can apply within and between groups, pointing to a future of collective intelligence via automated coordination and productive conflict – stemming from durable context and quantifiable preference data. The toolkit represents an advance on current SOTA large-group discourse software by leveraging cryptography (for private sessions and content), AI (for input UX and result interpretation), and decentralized protocols (for censorship-resistance and data permanence).
