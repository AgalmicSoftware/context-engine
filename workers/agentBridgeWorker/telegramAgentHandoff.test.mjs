...
test('Telegram agent handoff skill is packaged with the worker', () => {
  ...
  assert.match(wrapped, /\*\*Skill version:\*\* 2026-06-26 \(wrapped-v8\)/);
  ...
  assert.match(wrapped, /Read the skill \(no search\):/);
  assert.match(wrapped, /`Invite Token: \.\.\.`/);
  assert.match(wrapped, /When invoked from a short forwarded prompt/);
  assert.match(wrapped, /do not inspect local skills, local\s+files, previous sessions, memories, recovered scripts, Geo, or Index first/);
  ...
  assert.match(wrapped, /version includes `wrapped-v8`/);
...
test('Telegram agent handoff exposes the dedicated Agent Village Wrapped skill metadata', async () => {
  ...
  assert.equal(response.status, 200);
  assert.equal(body.ok, true);
  assert.equal(body.version, '2026-06-26 (wrapped-v8)');
...
test('Telegram agent handoff serves a short Agent Village Wrapped skill alias to HEAD probes', async () => {
  const response = await handleTelegramAgentHandoffRequest({
    request: agentRequest('/wrapped', { token: '', method: 'HEAD' }),
    env: baseEnv(),
  });
...
test('Agent Village Wrapped invite onboarding mints wrapped agent-only credential metadata', async () => {
  const env = multiTelegramOnlyEnv({
    defaultSessionSlug: 'agent-village-wrapped',
    sessions: ['agent-village-wrapped'],
    overrides: {
      AGENT_BRIDGE_AGENT_API_TOKEN: '',
      AGENT_BRIDGE_PUBLIC_URL: 'https://bridge.example',
      AGENT_BRIDGE_AGENT_ONLY_TOKEN_TTL_SECONDS: '604800',
  ...
  assert.equal(loaded.record.sessionSlug, 'agent-village-wrapped');
  assert.equal(loaded.record.username, '');
  assert.equal(loaded.record.ttlSeconds, 604800);
...
test('Telegram session-meta validates slug and CORS preflight', async () => {
  const env = telegramOnlyEnv();
  const missingResponse = await handleTelegramAgentHandoffRequest({
    request: new Request('https://bridge.example/telegram/agent/api/session-meta', {
      headers: { origin: 'https://client.example' },
    }),
    env,
  });
