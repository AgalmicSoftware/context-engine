...
export function resolveAgentBridgeDeployConfig({
  ...
  const config = {
    ...
    accountId: safeString(flags['account-id'] || env.CLOUDFLARE_ACCOUNT_ID),
    workerName,
    workersSubdomain,
    ...
    vars: {
      ...
      DEFAULT_CHAIN_ID: safeString(flags['default-chain-id'] || env.DEFAULT_CHAIN_ID || DEFAULT_CHAIN_ID),
      DEFAULT_RPC_URL: safeString(flags['default-rpc-url'] || env.DEFAULT_RPC_URL),
...
export function validateAgentBridgeDeployConfig(config = {}) {
  ...
  if (!safeString(config.accountId)) missing.push('CLOUDFLARE_ACCOUNT_ID or account lookup via token');
  ...
  if (!/^https:\/\/[^/]+\.workers\.dev$/i.test(safeString(config.vars?.AGENT_BRIDGE_PUBLIC_URL))) {
...
export function buildAgentBridgeDeployPlan(config = {}) {
  ...
  return {
    ...
    remainingDirectApiCalls: [
      {
        order: 1,
        purpose: 'Resolve account when CLOUDFLARE_ACCOUNT_ID is not supplied',
        method: 'GET',
