...
Adaptive execution. Use the same endpoints and answer every fetched statement in every model. If constrained, use one private helper script for onboarding, fetch, prediction, answer batches, both vote modes, and image so run_id cannot be lost across tool calls. Keep prediction JSON in local files and make stdout at most one compact status JSON with counts and image_url. Never print statement text/options, full payloads, retries, debug logs, or per-row predictions into chat. Omit rationales, batch 10-15 statements, reason over local indexes, and map to ids in code. This is output-control only; do not change submitted answer/vote/image schema.
