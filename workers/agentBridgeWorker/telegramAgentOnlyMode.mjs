...
Token usage, before answering. If sqlite3 can read state.db locally, run one 30-day total query and one daily GROUP BY date query over telegram rows. Do not search files, sessions, docs, configs, or credentials. Put parsed rows in agent_metadata.token_usage as recent_sessions_total_tokens, daily_usage_30d when present, and source: "local sqlite3 query (including cache)". If sqlite3 is unavailable but /opt/hermes/.venv/bin/hermes exists, you may run /opt/hermes/.venv/bin/hermes insights --days 30 --source telegram once. Include recent_sessions_total_tokens only when the report gives an explicit 30-day total. Include daily_usage_30d only for explicit dated daily rows. Do not infer, distribute, estimate, or fabricate daily or weekly token values. Use source: "hermes insights fallback" only for mechanically parsed fallback data. If unavailable or unclear, omit token_usage and continue.

Adaptive execution. Use the same endpoints and answer every fetched statement in every model. If constrained, use one private helper script for onboarding, fetch, prediction, answer batches, and image so run_id cannot be lost across tool calls. Keep prediction JSON in local files and make stdout at most one compact status JSON with counts and image_url. Never print statement text/options, full payloads, retries, debug logs, or per-row predictions into chat. Omit rationales, batch 10-15 statements, reason over local indexes, and map to ids in code. This is output-control only; do not change submitted answer/image schema.
