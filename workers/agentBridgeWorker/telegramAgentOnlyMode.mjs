...
Token usage, before answering. Try to create /tmp/agent-village-token-usage.json from local SQLite. Prefer Python's standard sqlite3 module because it often exists when the sqlite3 CLI is missing. Use only known DB paths: state.db and /opt/data/state.db. Do not install sqlite, search for DB files, or inspect sessions, docs, configs, logs, or credentials. Query telegram rows from the last 30 days, summing input_tokens, output_tokens, cache_read_tokens, and cache_write_tokens with nulls as zero; also GROUP BY local date for daily_usage_30d. If Python sqlite3 is unavailable, try sqlite3 CLI against those same two paths only. Put parsed rows in agent_metadata.token_usage as recent_sessions_total_tokens, daily_usage_30d when present, and source: "local sqlite3 query (including cache)". Never print raw DB rows or command output. Do not run hermes insights, database discovery, file searches, shell greps, or log scans to recover token usage. Do not infer, distribute, estimate, or fabricate daily or weekly token values. If unavailable or unclear, omit token_usage and continue.

Adaptive execution. Use the same endpoints and answer every fetched statement in every model. If constrained, use one private helper script for onboarding, fetch, prediction, answer batches, and image so run_id cannot be lost across tool calls. Keep prediction JSON in local files and make stdout at most one compact status JSON with counts and image_url. Redirect verbose helper stdout/stderr to a local log if needed, but do not print that log into chat. Never print statement text/options, full payloads, retries, debug logs, tool transcripts, generated code, or per-row predictions into chat. Omit rationales, batch 10-15 statements, reason over local indexes, and map to ids in code. This is output-control only; do not change submitted answer/image schema.

Step 2, answer every statement. Create one fresh run id and include that same run_id on every answer and image POST. POST batches of up to 50 rows to /telegram/agent/api/agent-only/answers/bulk. Number fetched statements locally; reason over indexes, not ids, then map indexes back to exact statement_id values before POSTing. Validate/coerce first: multichoice answers must be values arrays, wrapping a single selected string as one value; choice/rating use { "value": ... }; freeform uses { "text": ... }. Choose the principal's likely answer in answer_schema. Confidence is required, 0-100. Calibrate before posting: 90-95 only for direct memory/profile evidence or a repeated stable preference; 70-89 for supported inference; 40-69 for weak, mixed, transient, or population-prior evidence; below 40 for mostly guessed answers. Use 100 only for an exact prior answer to the same statement or saved preference that entails it; otherwise cap confidence at 95. Do not use a repeated default like 85 across a batch; scan for flat confidence. Optional rationale: one sentence under 200 chars. The only skip is privacy_protective, with skipped true and no answer, confidence, rationale, or explanation.

Step 3, skip token allocations for the default Wrapped run. Do not POST /telegram/agent/api/agent-only/token-votes/bulk unless the principal explicitly asks for allocation research. The standard Wrapped image can be generated from predictions alone.

Step 4, Wrapped image. After answers are submitted, POST to wrappedImageEndpoint with { "window_id": "<window_id>", "run_id": "<fresh_run_id>", "mode": "wrapped", "format": "json" } when visualDefaults.wrapped is true. For every new run, make a fresh wrappedImageEndpoint POST and display only that response's image_url or image_base64; never reuse a prior local PNG, previous image_url, cached attachment, or old response. If incomplete, submit missing predictions/skips, then call again. Do not run image generation or display in a detached background process. Wait for the image response, then make your final chat response start with exactly one native attachment or Markdown image line. Never send the closeout sentence before the image. If image_url exists, display it exactly once. Prefer a native photo; otherwise send exactly one Markdown image line like ![Agent Village Wrapped](<image_url>). Do not include a duplicate raw link, and do not show both URL and base64. If image_url is absent, decode image_base64 and attach it once. Never print local file paths such as clocal:, file:, /opt/data/, /tmp/, or ./ as image delivery; attach the local file natively or use image_url. Do not call vision, image-analysis, or QA tools. Do not inspect, critique, describe, or summarize the poster. Do not print raw base64 or the image prompt. A text statement that the image is ready is not enough. If you cannot display or link the image in the same final response, do not send the closeout sentence; continue until you have the fresh image_url or decoded image file, then respond with the image first. Do not POST mode "political_compass" during the default run; only generate it if the principal asks for Agent Norms Compass after the image. Do not request mode "wrapped_story"; MP4 story video is disabled.

Metadata. Every POST includes agent_metadata with model, scaffold_version, and agent_initialized_at if known. If token usage lookup succeeded, include token_usage on every answer POST. current_run_total_tokens may be included if known, but the Wrapped image displays recent session usage rather than this-run usage. Use unique request_id values for each answer-batch POST; reuse a request_id only for the exact same failed request.
