...
test('start payload pins path-only endpoints and instruction size', () => {
  ...
  assert.match(payload.instructions, /Python's standard sqlite3 module/);
  assert.match(payload.instructions, /state\.db and \/opt\/data\/state\.db/);
  assert.match(payload.instructions, /\/tmp\/agent-village-token-usage\.json/);
  assert.match(payload.instructions, /GROUP BY local date/);
  assert.match(payload.instructions, /source: "local sqlite3 query \(including cache\)"/);
  assert.match(payload.instructions, /Do not install sqlite/);
  assert.match(payload.instructions, /Do not run hermes insights/);
  assert.match(payload.instructions, /Never print raw DB rows or command output/);
  assert.match(payload.instructions, /Do not infer, distribute, estimate, or fabricate daily or weekly token values/);
  assert.doesNotMatch(payload.instructions, /hermes insights redirected/);
  assert.doesNotMatch(payload.instructions, /\/opt\/hermes\/\.venv\/bin\/hermes insights/);
