...
test('start payload pins path-only endpoints and instruction size', () => {
  ...
  assert.equal(payload.statementEndpoint, '/api/agent/agent-only/statements');
  assert.equal(payload.answerEndpoint, '/api/agent/agent-only/answers/bulk');
  assert.equal(payload.voteEndpoint, '/api/agent/agent-only/token-votes/bulk');
  assert.equal(payload.wrappedImageEndpoint, '/api/agent/agent-only/wrapped-image');
  ...
  assert.match(payload.instructions, /!\[Session Wrapped\]\(<image_url>\)/);
...
test('wrapped image prompt uses importance wording and suppresses decorative text', () => {
  ...
  assert.match(prompt, /compact top-left "Session Lab" wordmark/);
  assert.match(prompt, /What your agent thinks it knows about you/);
  assert.match(prompt, /Do not render the word "Wrapped"/);
  assert.match(prompt, /"SESSION" is heavy uppercase block sans/);
  assert.match(prompt, /attached Session Lab logo image as the style reference/);
  assert.match(prompt, /"LAB" is elegant high-contrast serif/);
  assert.match(prompt, /lay "SESSION" and "LAB" side-by-side/);
...
test('wrapped image prompt supports Agent Norms Compass mode around the most-important question', () => {
  ...
  assert.match(prompt, /Session Lab Norms Compass poster/);
...
test('wrapped image prompt has a neutral safety retry variant for compass mode', () => {
  ...
  assert.match(prompt, /Session Lab Norms Map/);
  assert.match(prompt, /neutral product-research language/);
  assert.doesNotMatch(prompt, /Norms Compass poster/);
  assert.doesNotMatch(prompt, /political compass/i);
});

test('Session Wrapped skill fixture stays generic and compact', () => {
  const skill = readFileSync(new URL('./skills/ce-session-wrapped/SKILL.md', import.meta.url), 'utf8');
  assert.match(skill, /Small fixture question bank for dry-run planning only/);
  assert.match(skill, /fixture-q-1/);
  assert.match(skill, /fixture-q-2/);
  assert.match(skill, /fixture-q-3/);
  assert.doesNotMatch(skill, /Agent guess:|favorite book|favorite movie|movie or TV show|favorite game|p\(bloom\)|AI Optimism score/i);
  const formerEventPattern = new RegExp(`${['agent', 'village'].join('[- ]')}|${['agent', 'village', 'wrapped'].join('-')}`, 'i');
  assert.doesNotMatch(skill, formerEventPattern);
});
