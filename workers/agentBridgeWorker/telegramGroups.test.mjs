...
test('Telegram group defaults include Edge demo bucket categories', () => {
  ...
  assert.equal(ids.includes('time_in_crypto'), true);
  assert.equal(ids.includes('primary_focus'), true);
...
test('Telegram bucket memberships are durable and readable by managed wallet address', async () => {
  ...
  assert.deepEqual(bucketRecord.selections.primary_focus, ['governance']);
