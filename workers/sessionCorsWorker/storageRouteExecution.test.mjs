...
const WORKER_GROUP_SESSION_ID = '0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa';
...
test('storageRoute consumes the session-secret RPC for worker-canonical SBT gates without adding it to public config', async () => {
	...
	const response = await storageRoute({
		...
		deps: {
			...
			resolveRpcUrlListForGate: (runtimeConfig, gateChainId) =>
...
test('storageRoute rejects non-canonical chain ids on direct gates, access conditions, and upload policies', async () => {
	const invalidChainIds = ['3.1337e4', '31337.0', '-31337', '0x7a69junk', '9007199254740993'];
	const sbtAddress = '0x00000000000000000000000000000000000000aa';
...
test('storageRoute enforces worker group gates and group upload allowlists', async () => {
	...
	const groupMembershipDeps = {
		isWorkerGroupMember: readWorkerGroupMembershipProjection,
	};
	...
	const deniedUpload = await storageRoute({
		...
		authScopes: {},
		baseHeaders: {},
		deps: { json, randomBytes: fixedRandomBytes, ...groupMembershipDeps },
	});
	...
	const allowedPolicyUpload = await storageRoute({
		...
		authScopes: {},
		baseHeaders: {},
		deps: { json, randomBytes: createSequenceRandomBytes(33), ...groupMembershipDeps },
	});
...
test('storageRoute read-normalizes legacy payloadAccessMode metadata rows into v2 list metadata', async () => {
	...
	const env = { CE_STORAGE_INDEX_KV: kv };
	await kv.put(
		`ce-storage:session-a:questions:${CF_ID}`,
		JSON.stringify({
			id: CF_ID,
			backend: 'cloudflare',
			resource: 'questions',
...
test('storageRoute filters Cloudflare list rows by per-item access conditions', async () => {
	...
	await kv.put(
		`ce-storage:session-a:questions:public-row`,
		JSON.stringify({
			id: 'public-row',
			backend: 'cloudflare',
			resource: 'questions',
	...
	await kv.put(
		`ce-storage:session-a:questions:admin-row`,
		JSON.stringify({
			id: 'admin-row',
			backend: 'cloudflare',
			resource: 'questions',
	...
	assert.equal(listResponse.status, 200);
	assert.deepEqual(
		listed.items.map((item) => item.storageRef.id),
		['public-row'],
	);
...
test('storageRoute returns bounded cursor pages and filters every page independently', async () => {
	const rows = new Map([
		[
			'ce-storage:session-a:questions:denied-first',
			JSON.stringify({
				id: 'denied-first',
				backend: 'cloudflare',
				resource: 'questions',
		...
		[
			'ce-storage:session-a:questions:allowed-second',
			JSON.stringify({
				id: 'allowed-second',
				backend: 'cloudflare',
				resource: 'questions',
		...
		[
			'ce-storage:session-a:questions:denied-second',
			JSON.stringify({
				id: 'denied-second',
				backend: 'cloudflare',
				resource: 'questions',
	...
	const index = {
		async list(options) {
			...
			return {
				keys: [{ name: 'ce-storage:session-a:questions:allowed-second' }, { name: 'ce-storage:session-a:questions:denied-second' }],
				list_complete: true,
			};
		},
		async get(key) {
	...
	const secondResponse = await storageRoute({
		...routeArgs,
		request: new Request('https://worker.example/storage/list?resource=questions&cursor=page-two&limit=9999'),
	});
	const secondPage = await readJson(secondResponse);
	assert.equal(secondResponse.status, 200);
	assert.deepEqual(
		secondPage.items.map((item) => item.storageRef.id),
		['allowed-second'],
	);
...
test('storageRoute accepts POST list cursors and gives query paging values precedence', async () => {
	...
	const queryPage = await storageRoute({
		...routeArgs,
		request: new Request('https://worker.example/storage/list?resource=questions&cursor=query-cursor&limit=2', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
...
test('storageRoute accepts bare Cloudflare role_gate storage access for configured worker roles', async () => {
	...
	await kv.put(
		`ce-storage:session-a:questions:review-row`,
		JSON.stringify({
			id: 'review-row',
			backend: 'cloudflare',
			resource: 'questions',
...
test('storage envelope audits ignore D1 aliases and keep KV authoritative', async () => {
	const d1BindingNames = ['CE_STORAGE_AUDIT_D1', 'STORAGE_AUDIT_D1', 'D1', 'DB'];
...
test('storageRoute normalizes legacy key metadata without rewrapping before a new envelope write', async () => {
	...
	const [oldRead, newRead] = await Promise.all([read(first.body.id, 'legacy-source-read'), read(upgraded.body.id, 'legacy-upgrade-read')]);
...
test('storageRoute worker_envelope denies conditions before key unwrap', async () => {
	...
	assert.equal(denied.reason, 'worker_role_denied');
	assert.equal(
		[...kv.store.keys()].some((key) => key.startsWith('ce-storage-audit:')),
		false,
	);
});
...
test('storageRoute worker_envelope evaluates any/all and supported condition kinds', async () => {
	...
	let config = createEnvelopeConfig({
		config: { sessionId: WORKER_GROUP_SESSION_ID },
	});
	const setConfig = (nextConfig) => {
		config = nextConfig;
	};
