  ...
  withTempRepo((rootDir) => {
    writeFile(
      ...
      JSON.stringify({
        scripts: {
          ...
          'verify:release':
            'npm run lint && npm run typecheck:client && npm run verify:release-version && npm run -s test:node:tracked && npm run test:release:client && npm run verify:public-release-surface && npm run verify:public-assets && npm run worker:bundle && npm run verify:worker-bundle && npm --prefix client run build',
        },
      }),
    );
    writeFile(
    ...
    writeFile(
      ...
      [
        ...
        '  - run: echo "${{ steps.meta.outputs.app_version }}"',
