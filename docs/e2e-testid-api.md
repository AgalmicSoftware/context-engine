...
Component: `client/src/components/E2E/DevE2eNav.jsx`
...
The `/about` route renders `client/src/components/About/AboutPage.jsx`.
The `/` home route renders `client/src/components/MainContent/MainAreaTabs.jsx`, which now mounts `ToolExplorer.jsx` as the tool-launcher tab and `OnboardingWalkthrough.jsx` as the welcome tab.
...
| --- | --- | --- | --- | --- |
| `ce-wallet-display` | `client/src/components/Navbar/AccountDisplay.jsx` | Logged-in account display button. | yes | `data-ce-wallet-address` |

The navbar-mounted account/login/settings surface that owns the shared modal lives in `client/src/components/Account/LoginAndSettingsModal.jsx`; `client/src/components/Navbar/AccountSection.jsx` mounts it.

## AdminPage (`/admin`)

| `data-testid` | Component path(s) | Meaning / When Present | TestID API |
| --- | --- | --- | --- |
| `ce-admin-session-select` | `client/src/components/Admin/AdminPage.jsx` | Session dropdown selector. | yes |
| `ce-admin-gate-mode-select` | `client/src/components/Admin/AdminPage.jsx` | Default gate mode selector (ANY/ALL). | yes |
| `ce-admin-gate-update-button` | `client/src/components/Admin/AdminPage.jsx` | "Update default gate on-chain" button. | yes |
| `ce-admin-gate-status` | `client/src/components/Admin/AdminPage.jsx` | Status note for default-gate sync (contains success/error text). | yes |
| `ce-admin-not-admin-warning` | `client/src/components/Admin/AdminPage.jsx` | Warning note shown when the connected wallet is not the session admin. | yes |
...
| `data-testid` | Component path(s) | Meaning / When Present | TestID API |
| --- | --- | --- | --- |
| `ce-sponsor-worker-url-toggle` | `client/src/components/Sponsor/SponsorPage.jsx` | Toggle that unlocks or hides the manual source-worker URL override field. | yes |
| `ce-sponsor-worker-url` | `client/src/components/Sponsor/SponsorPage.jsx` | Manual source-worker URL override input shown after the worker URL toggle is opened. | yes |
| `ce-sponsor-create` | `client/src/components/Sponsor/SponsorPage.jsx` | Primary button that issues grants, uploads the encrypted bundle, and generates the share URL. | yes |
| `ce-sponsor-status` | `client/src/components/Sponsor/SponsorPage.jsx` | Status note for sponsor bundle creation progress/success/error text. | yes |
| `ce-sponsor-share-url` | `client/src/components/Sponsor/SponsorPage.jsx` | Read-only generated `/new?sponsored=...#k=...` share URL input. | yes |
| `ce-sponsor-tx-id` | `client/src/components/Sponsor/SponsorPage.jsx` | Read-only Arweave transaction id status line for the generated sponsor bundle. | yes |

## CompareAddresses

| `data-testid` | Component path(s) | Meaning / When Present | TestID API |
| --- | --- | --- | --- |
| `ce-compare-address-a` | `client/src/components/UserPage/CompareAddresses.jsx` | Primary compare address input A (index 0). | yes |
| `ce-compare-address-b` | `client/src/components/UserPage/CompareAddresses.jsx` | Primary compare address input B (index 1). | yes |
| `ce-compare-add-address` | `client/src/components/UserPage/CompareAddresses.jsx` | Adds an additional address input row. | yes |
| `ce-compare-run` | `client/src/components/UserPage/CompareAddresses.jsx` | Runs the comparison. | yes |
| `ce-compare-result` | `client/src/components/UserPage/CompareAddresses.jsx` | Root wrapper for the results section (Collapse). | yes |
| `ce-compare-agreements` | `client/src/components/UserPage/CompareAddresses.jsx` | Agreements list container. | yes |
| `ce-compare-disagreements` | `client/src/components/UserPage/CompareAddresses.jsx` | Disagreements list container. | yes |

## PolisReport

| `data-testid` | Component path(s) | Meaning / When Present | TestID API | Disambiguators |
| --- | --- | --- | --- | --- |
| `ce-polis-report-root` | `client/src/components/PolisReport/PolisReport.jsx` | Root wrapper of the PolisReport component. | yes |  |
| `ce-polis-settings-toggle` | `client/src/components/PolisReport/PolisReport.jsx` | Settings (cog) toggle button that opens the settings row. | yes |  |
| `ce-polis-demo-data-toggle` | `client/src/components/PolisReport/PolisReport.jsx` | "Demo Data" checkbox input. | yes |  |
| `ce-polis-analyze-clusters` | `client/src/components/PolisReport/PolisReport.jsx` | "Analyze clusters" button. | yes |  |
| `ce-polis-cluster-analysis` | `client/src/components/PolisReport/PolisReport.jsx` | Cluster analysis summary container. | yes | `data-ce-cluster-index`, `data-ce-analysis-state` |

## OnePageSession (Session Results Toggle)

| `data-testid` | Component path(s) | Meaning / When Present | TestID API |
| --- | --- | --- | --- |
| `ce-session-results-toggle` | `client/src/components/OnePageSession/OnePageSession.jsx` | Header/toggle that expands the "Results" section on a session page. | yes |
| `ce-session-pile-back` | `client/src/components/OnePageSession/OnePageSession.jsx` | Back control shown while full questions view is open; returns to pile mode. | yes |
| `ce-session-auto-mint-login-banner` | `client/src/components/OnePageSession/OnePageSession.jsx` | Sticky warning banner shown when an auto-mint URL is present but the viewer must log in first. | yes |
| `ce-session-auto-mint-countdown` | `client/src/components/OnePageSession/OnePageSession.jsx` | Countdown banner shown immediately before the session page runs the auto-mint queue. | yes |
| `ce-session-auto-mint-status` | `client/src/components/OnePageSession/OnePageSession.jsx` | Per-SBT status alert for session auto-mint progress/results (`data-ce-sbt-address`, `data-ce-status`). | yes |

## Agent Mode (Dev/E2E)

Agent Mode is a dev/e2e-only global interface `window.__ceAgent` (installed from `client/src/components/App.jsx` and implemented in `client/src/utilities/ceAgent.js`).

Agent page UI: `client/src/components/Agent/AgentPage.jsx`
...
| `ce-survey-answer-input` | `client/src/components/Shared/AudioInput/AudioInput.jsx` (wired from `SurveyTool.jsx`) | Textarea used for a freeform question answer. | yes | `data-ce-question-id` |
| `ce-survey-submit` | `client/src/components/SurveyTool/SurveyTool.jsx` | Primary submit button (header, footer, or pile view). | yes |  |
| `ce-survey-create-toggle` | `client/src/components/SurveyTool/SurveyTool.jsx` | Header create icon toggle (open/close CreateSurvey panel). | yes |  |
...
| `ce-survey-pile-hologram-panel` | `client/src/components/SurveyTool/PileHologramAssistant.jsx` | Full-card light-blue hologram face takeover rendered while the pile hologram toggle is active. | yes |  |
...
| `ce-survey-additional-input` | `client/src/components/Shared/AudioInput/AudioInput.jsx` (wired from `SurveyTool.jsx`) | Textarea used for additional comments. | yes | `data-ce-question-id` |
...
| `ce-question-filter-modal` | `client/src/components/SurveyTool/QuestionFilter.jsx` | Root wrapper for the QuestionFilter panel (inline and modal variants). | yes |  |
| `ce-question-filter-section-ai` | `client/src/components/SurveyTool/QuestionFilter.jsx` | Collapsible section header for AI filter controls. | yes |  |
| `ce-question-filter-section-sbt` | `client/src/components/SurveyTool/QuestionFilter.jsx` | Collapsible section header for SBT filter controls. | yes |  |
| `ce-question-filter-ai-query` | `client/src/components/SurveyTool/QuestionFilter.jsx` (via `AudioInput.jsx`) | AI filter query textarea. | yes |  |
| `ce-question-filter-ai-top-n` | `client/src/components/SurveyTool/QuestionFilter.jsx` | Top-N numeric input for AI question ranking. | yes |  |
| `ce-question-filter-ai-apply` | `client/src/components/SurveyTool/QuestionFilter.jsx` | Apply button for AI filter ranking. | yes |  |
| `ce-question-filter-clear-all` | `client/src/components/SurveyTool/QuestionFilter.jsx` | Clears all active question filters. | yes |  |

## SBTSelector (QuestionFilter + Other SBT Flows)

| `data-testid` | Component path(s) | Meaning / When Present | TestID API | Disambiguators |
| --- | --- | --- | --- | --- |
| `ce-sbt-selector-root` | `client/src/components/SBTs/SBTSelector.jsx` | Root wrapper for a selector instance. | yes | `data-ce-sbt-selector-id` |
| `ce-sbt-selector-manual-toggle` | `client/src/components/SBTs/SBTSelector.jsx` | Toggle button that shows/hides manual-by-address input mode. | yes |  |
| `ce-sbt-selector-manual-input` | `client/src/components/SBTs/SBTSelector.jsx` | Manual SBT address input field. | yes |  |
| `ce-sbt-selector-manual-add` | `client/src/components/SBTs/SBTSelector.jsx` | Manual SBT add button. | yes |  |
| `ce-sbt-selector-selected` | `client/src/components/SBTs/SBTSelector.jsx` | A selected SBT chip rendered in the selector. | yes | `data-ce-sbt-address` |

## SingleQuestionResponse (Decrypting Existing Responses)

| `data-testid` | Component path(s) | Meaning / When Present | TestID API | Disambiguators |
| --- | --- | --- | --- | --- |
| `ce-encrypted-answer-notice` | `client/src/components/SurveyTool/SingleQuestionResponse.jsx` | "Answer is encrypted" notice for a response field. | yes | `data-ce-question-id` |
| `ce-decrypt-answer` | `client/src/components/SurveyTool/SingleQuestionResponse.jsx` | Button that decrypts an encrypted answer. | yes | `data-ce-question-id` |
| `ce-encrypted-additional-notice` | `client/src/components/SurveyTool/SingleQuestionResponse.jsx` | "Additional comments are encrypted" notice. | yes | `data-ce-question-id` |
| `ce-decrypt-additional` | `client/src/components/SurveyTool/SingleQuestionResponse.jsx` | Button that decrypts encrypted additional comments. | yes | `data-ce-question-id` |

## CreateSurvey (Authoring)

| `data-testid` | Component path(s) | Meaning / When Present | TestID API | Disambiguators |
| --- | --- | --- | --- | --- |
| `ce-create-panel` | `client/src/components/SurveyTool/CreateSurvey.jsx` | Root element of the CreateSurvey panel. | yes |  |
| `ce-create-mode-switch` | `client/src/components/SurveyTool/CreateSurvey.jsx` | Switch between Manual and "from URL / Content" modes (when available). | yes |  |
| `ce-create-clear` | `client/src/components/SurveyTool/CreateSurvey.jsx` | Clear form button (when visible). | yes |  |
| `ce-create-title` | `client/src/components/SurveyTool/CreateSurvey.jsx` | Title input for Survey mode. | yes |  |
| `ce-create-question` | `client/src/components/SurveyTool/CreateSurvey.jsx` | Per-question container in the authoring form. | yes | `data-ce-question-index` |
| `ce-create-question-prompt` | `client/src/components/SurveyTool/CreateSurvey.jsx` | Question prompt textarea within a question container. | yes |  |
| `ce-create-question-tag-input` | `client/src/components/SurveyTool/CreateSurvey.jsx` | Tag input within a question container. | yes |  |
| `ce-create-question-add-tag` | `client/src/components/SurveyTool/CreateSurvey.jsx` | "Add Tag" control (checkmark) within a question container (only visible when input is non-empty). | yes |  |
| `ce-create-question-add-option` | `client/src/components/SurveyTool/CreateSurvey.jsx` | "Add Option" control for multichoice questions. | yes |  |
| `ce-create-question-single-select` | `client/src/components/SurveyTool/CreateSurvey.jsx` | Checkbox that enables single-select multichoice mode. | yes |  |
| `ce-create-submit` | `client/src/components/SurveyTool/CreateSurvey.jsx` | Primary submit button for Create Questions / Create Survey. | yes |  |
| `ce-create-success` | `client/src/components/SurveyTool/CreateSurvey.jsx` | Success confirmation wrapper shown after authoring submit completes. | yes |  |
| `ce-create-uploaded-questions` | `client/src/components/SurveyTool/CreateSurvey.jsx` | Wrapper around the list of uploaded questions (when present). | yes |  |
| `ce-create-uploaded-question` | `client/src/components/SurveyTool/CreateSurvey.jsx` | Per-uploaded-question list item in the success UI (when present). | yes | `data-ce-question-id` |
...
| `data-testid` | Component path(s) | Meaning / When Present | TestID API | Disambiguators |
| --- | --- | --- | --- | --- |
| `ce-gate-lock` | `client/src/components/Gates/GateMultiSelectLock.jsx` | Root element for the multi-select gate lock UI. | yes |  |
| `ce-gate-lock-button` | `client/src/components/Gates/GateMultiSelectLock.jsx` | Lock/unlock button that opens the gate picker popover. | yes |  |
| `ce-gate-lock-popover` | `client/src/components/Gates/GateMultiSelectLock.jsx` | Popover/dialog that lists available gates. | yes |  |
| `ce-gate-lock-row` | `client/src/components/Gates/GateMultiSelectLock.jsx` | A single gate row within the popover. | yes | `data-ce-gate-id` |
...
| `ce-database-save-docs-toggle` | `client/src/components/SurveyTool/SurveyGenerator/SurveyGenerator.jsx` | Checkbox shown in Tool Explorer `Data` add mode once queued extra URL/file sources exist; enables saving those queued sources into the session doc library on Generate. | yes |
...
| `data-testid` | Component path(s) | Meaning / When Present | TestID API | Disambiguators |
| --- | --- | --- | --- | --- |
| `ce-doc-panel` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Root element for the Doc Library panel. | yes |  |
| `ce-doc-title` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Panel title. | yes |  |
| `ce-doc-refresh` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Refresh button for listing docs. | yes |  |
| `ce-doc-upload-file-input` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | File input used to select a file for upload. | yes |  |
| `ce-doc-upload-file-button` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Upload button for file uploads. | yes |  |
| `ce-doc-url-input` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | URL input for uploading a link record. | yes |  |
| `ce-doc-url-title-input` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Optional title input for link record uploads. | yes |  |
| `ce-doc-url-add-button` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Button that uploads a link record. | yes |  |
| `ce-doc-lock-toggle` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Lock/unlock toggle for plaintext vs Lit-encrypted upload mode. | yes | `data-ce-locked` |
| `ce-doc-audience-session-gate` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Radio input selecting the session `docUploads` gate as the Lit audience. | yes |  |
| `ce-doc-audience-custom` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Radio input selecting custom SBT audience. | yes |  |
| `ce-doc-custom-sbt-input` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Input for adding custom SBT addresses to the audience. | yes |  |
| `ce-doc-custom-sbt-add` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Button that adds a custom SBT address to the audience list. | yes |  |
| `ce-doc-custom-mode-any` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Radio input selecting "Any" (OR) mode for custom audience list. | yes |  |
| `ce-doc-custom-mode-all` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Radio input selecting "All" (AND) mode for custom audience list. | yes |  |
| `ce-doc-row` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Row wrapper for a single doc entry in the list. | yes | `data-ce-doc-txid`, `data-ce-doc-storage`, `data-ce-doc-kind`, `data-ce-index-status` |
| `ce-doc-row-view` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Row action that opens the viewer modal for a doc entry. | yes |  |
| `ce-doc-row-open-arweave` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Row action link that opens the Arweave gateway URL. | yes |  |
| `ce-doc-viewer` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Viewer modal root. | yes |  |
| `ce-doc-viewer-title` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Viewer modal header/title area. | yes |  |
| `ce-doc-viewer-error` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Viewer modal error text wrapper (when present). | yes |  |
| `ce-doc-viewer-download` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Download link shown for blob-backed previews (when present). | yes |  |
| `ce-doc-viewer-pdf` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | PDF iframe preview (when present). | yes |  |
| `ce-doc-viewer-image` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Image preview `<img>` (when present). | yes |  |
| `ce-doc-viewer-text` | `client/src/components/DocumentLibrary/DocumentLibraryPanel.jsx` | Text/JSON `<pre>` content preview (when present). | yes |  |
