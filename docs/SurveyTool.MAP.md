...
SurveyTool.tsx  [top-level wrapper]
  -> SurveySelector.tsx  [survey/questions selector + filter + results]
     ...
     -> SurveyResults.tsx  [survey/question results runtime and execution owner]
        ...
        -> surveyResultsExportDisplayHelpers.ts  [HTML report readiness plans and state patches]
...
| `surveyResultsExportPlans.ts` | Results export labels and plans | Builds export labels, control display descriptors, base filenames, generation descriptors, and inert download plans without owning export payload generation, Blob/browser download execution, route, cache, fetch, decrypt, or state application |
...
| `surveyResultsExportDisplayHelpers.ts` | HTML report readiness/settlement planning | Builds typed HTML report analysis payload descriptors, HTML report modal display descriptors, modal open/close, section toggle, demo-mode, export-format, and analysis status state-patch descriptors, section availability/readiness plans, blocked/ready download attempt descriptors, and success/failure patch descriptors without owning AI generation, cache reads/writes, artifact persistence, render execution, PDF/HTML generation, browser download, route, or state application |
