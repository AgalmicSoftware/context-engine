...
SurveyTool.tsx  [top-level wrapper]
  -> SurveySelector.tsx  [survey/questions selector + filter + results]
     ...
     -> SurveyResults.tsx  [survey/question results runtime and execution owner]
        ...
        -> surveyResultsHtmlReportModalDescriptor.ts  [HTML report modal display descriptors]
        -> surveyResultsExportDisplayHelpers.ts  [HTML report download attempt and settlement plans]
...
| `surveyResultsExportDisplayHelpers.ts` | HTML report download attempt/settlement planning | Builds blocked/ready download attempt descriptors and success/failure patch descriptors, with compatibility re-exports for split HTML report helper modules, without owning AI generation, cache reads/writes, artifact persistence, render execution, PDF/HTML generation, browser download, route, or state application |
