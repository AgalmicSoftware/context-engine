...
- Current lengths:
  ...
  - `surveyResultsCacheWriteEligibilityPlan.ts`: **80 lines**
...
SurveyTool.tsx  [top-level wrapper]
  -> SurveySelector.tsx  [survey/questions selector + filter + results]
     ...
     -> SurveyResults.tsx  [survey/question results runtime]
        ...
        -> surveyResultsCacheWriteEligibilityPlan.ts  [filter-bookmark cache-write eligibility plan]
...
| `surveyResultsCacheWriteEligibilityPlan.ts` | Filter-bookmark cache-write eligibility planning | Derives the parent-owned filter bookmark write target, blocked reason, payload shape, invalid-cache warning flag, and success-feedback decision from passed-in values while leaving cache reads, cache writes/persistence, timers, feedback state application, fetch, decrypt, export, route, polling, and mutation behavior in `SurveyResults` |
...
|---|---|---|
| SurveyResults first write-controller readiness | First narrow filter-bookmark write dispatch now runs through an injected-port controller after parent-owned eligibility planning. Parent still owns filter cache reads, warning/error logging, success feedback state/timer, and all broad persistence. | Keep this controller narrow; require same-stage recovery/regression coverage before expanding only within the same filter-bookmark boundary. |
| SurveyResults remaining write blockers | Survey/question bookmark writes now have behavior coverage, but they do not yet have a pure plan or injected-port controller. Report-analysis/export cache writes remain AI/export-adjacent and too broad for this lane. | Next lane should start with survey/question bookmark write planning tests and a pure plan before any injected write dispatch. |
