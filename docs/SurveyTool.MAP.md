...
- Current lengths:
  ...
  - `surveyResultsAnalysisLifecyclePlan.ts`: **171 lines**
...
|---|---|---|---|---|---|---|
| SurveyResults cache write boundaries | Filter bookmark, survey/question bookmark, selected fallback write, and analysis artifact cache-write dispatch use narrow injected-port controllers. | `surveyResultsCacheWriteEligibilityPlan.ts` defines target identities, blocked reasons, payload descriptors, state-patch descriptors, and outcome shapes from passed-in values; `surveyResultsAnalysisLifecyclePlan.ts` defines analysis artifact target identity, payload descriptors, blocked reasons, lifecycle status/result, cached-ready cleanup, and failure-recovery state patches from passed-in values. | Identity/payload matrices, write-readiness matrix, selected fallback/status coverage, broad-normalization blocker classification, and analysis artifact success, cached-ready cleanup, blocked/no-op, partial failure/retry, parent status output, stale artifact regeneration, missing identity no-op, plan-derived failure state patch, and no-download/fetch/decrypt side effects are pinned. | Cache reads, broad cache persistence, cache normalization, feedback timers, warning/error logging, AI generation, artifact merge, export/download execution, fetch, decrypt, route, polling, and state application. | Broad persistence still crosses cache reads, polling/status state, AI/export artifacts, artifact merge, and parent feedback lifecycles. Analysis artifact cache writes are controller-routed, analysis lifecycle status is pure-plan-only, broad results cache normalization is test-pinned only, and HTML report export/download persistence is blocked by DOM download and payload generation ownership. | Next safe lane: keep broad cache persistence parent-owned; the only near-term safe move is a tiny analysis lifecycle pre-controller around status/ordering with injected ports, while cache reads, AI generation, artifact merge, export/download, polling, and state application remain parent-owned. |
