...
- Current lengths:
  ...
  - `SurveyQuestions.tsx`: **8,876 lines**
...
| SurveyQuestions decrypt/cache readiness | Submit-status handoff and question-decrypt status planning have extracted controllers, but decrypt/cache restore execution is not controller-routed. | `surveyToolResponseSourceController.ts` and `surveyQuestionsJsonDerivation.ts` define restore/source and JSON-readiness decisions from passed-in values. | Encrypted submitted JSON readiness, same-envelope restore, refreshed-plaintext precedence, recent payload restore, stale recent-payload fallback to normal metadata loading, and disabled local-cache fallback are pinned. | Lit/decrypt execution, submit execution, worker/storage, cache writes, route/navigation, wallet/provider, and state mutation. | Source restore still meets real decrypt/cache/submit lifecycles inside the parent. | Add only source-restore helper extraction if it reduces duplicated decision logic; do not move Lit calls or cache/submit execution. |
| UserPage cache/decrypt/action boundaries | Analyze, bookmark, and cache-refresh clicks use injected action controllers. | `userPageActionController.ts` and UserPage helper modules define action/readiness/loading contracts from passed-in values. | Cached responses, gated visibility, no-account decrypt-click inertness, decrypt-click wiring, stale/refresh boundaries, and cached-analysis refresh boundaries are pinned. | Real decrypt execution, cache writes, bookmark persistence, AI fetch, route mutation, timers, and refresh execution. | Cache/decrypt refresh still crosses user visibility, fetch, analysis, and parent state. | Prefer tests or a pure display/eligibility plan only if it deletes parent decision code. |
