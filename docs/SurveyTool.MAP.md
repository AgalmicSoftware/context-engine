...
- Current lengths:
  ...
  - `surveyResultsAnalysisLifecyclePlan.ts`: **97 lines**
...
| `surveyResultsAnalysisLifecyclePlan.ts` | Analysis artifact lifecycle planning | Chooses current vs cached analysis artifacts, requested/covered/missing report sections, target identity, and ready/generate status patches from passed-in values while leaving cache reads, cache writes/persistence, AI generation, artifact merge, export/download execution, route, polling, timers, and state application in `SurveyResults` |
...
|---|---|---|---|---|---|---|
| SurveyResults cache write boundaries | Filter bookmark, survey/question bookmark, selected fallback write, and analysis artifact write dispatch use narrow injected-port controllers. | `surveyResultsCacheWriteEligibilityPlan.ts` defines target identities, blocked reasons, payload descriptors, state-patch descriptors, and outcome shapes from passed-in values; `surveyResultsAnalysisLifecyclePlan.ts` defines analysis artifact ready/generate lifecycle decisions from passed-in values. | Identity/payload matrices, write-readiness matrix, selected fallback/status coverage, and analysis artifact success, blocked/no-op, failure, retry, parent status output, stale artifact regeneration, missing identity no-op, and no-download side effects are pinned. | Cache reads, broad cache persistence, cache normalization, feedback timers, warning/error logging, AI generation, artifact merge, export/download execution, fetch, decrypt, route, polling, and state application. | Broad persistence still crosses cache reads, polling/status state, AI/export artifacts, artifact merge, and parent feedback lifecycles. Analysis artifact lifecycle planning is pure and extracted, but broad persistence is still parent-owned. | Next safe lane: keep broad cache persistence parent-owned; if more work is needed, use test-only recovery coverage or a tiny injected-port lifecycle wrapper only if it deletes parent decision code without moving cache reads, AI generation, artifact merge, export/download, polling, or state application. |
