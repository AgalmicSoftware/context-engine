...
SurveyTool.tsx  [top-level wrapper]
  -> SurveySelector.tsx  [survey/questions selector + filter + results]
     ...
     -> SurveyResults.tsx  [survey/question results runtime and execution owner]
        ...
        -> surveyResultsExportDisplayHelpers.ts  [HTML report readiness and settlement plans]
...
| `surveyResultsExportDisplayHelpers.ts` | HTML report readiness/settlement planning | Builds typed HTML report analysis payload descriptors, HTML report modal display descriptors, section availability/readiness plans, blocked/ready download attempt descriptors, and success/failure patch descriptors without owning AI generation, cache reads/writes, artifact persistence, render execution, PDF/HTML generation, browser download, route, or state application |
