...
| `surveyResultsExportPlans.ts` | Results export labels and plans | Builds export labels, control display descriptors, base filenames, generation descriptors, inert download plans, questions-only CSV payloads, and JSON export payloads without owning response CSV payload generation, Blob/browser download execution, route, cache, fetch, decrypt, or state application |
