...
| SurveyResults remaining write blockers | Broader cache persistence, normalization, and polling-coupled refresh orchestration are still deferred. | Next lane should stop before any controller unless identity, payload, failure/recovery, and lifecycle ownership are pinned. |
...
| SessionWizard publish/controller readiness | Publish summary worker/source display and manual metadata callback routing are now pinned without publish execution. Publish/controller extraction remains separate from SurveyResults cache-write work. | Tests first around one passive publish/readiness decision; avoid moving session mutation, route mutation, contract, worker, storage, or upload execution. |
