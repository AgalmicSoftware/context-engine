...
- Current lengths:
  ...
  - `surveyResultsCacheWriteEligibilityPlan.ts`: **171 lines**
...
| SurveyResults analysis/export artifact write readiness | Report-analysis/export artifact cache writes now have method-boundary coverage and a pure target/payload plan. Parent still owns cache reads, cache write execution, AI generation flow, artifact normalization/merge, export payload generation, and download execution. | A narrow injected controller is only safe if it keeps AI/export/cache-read behavior parent-owned and adds same-stage failure/recovery coverage. |
