...
| SBTPage realtime/cache readiness | Full-page mint/burn action rendering now sits in a passive action surface fed by parent-built descriptors and named execution props; holder count/refresh/scan-progress presentation now sits in `SbtPageHolderStatusDisplay.tsx` fed by typed holder-count and scan-progress descriptors; mini-card display state, admin/action status, mint-input display helpers, and central holder-refresh lifecycle planning are extracted, but realtime/cache execution is not controller-routed. | SBT helper modules define holder/action/status/session/mini-card readiness plans and the pure central-refresh lifecycle plan from passed-in values; `SbtPageFullActionButtons.tsx` consumes the full-action descriptors and routes only explicit mint/burn props through existing controllers; `SbtPageHolderStatusDisplay.tsx` defines the passive holder count and scan-progress display contracts consumed by `SbtPageStatsSection.tsx`. | Complete cached-metadata readiness without refresh/direct metadata/cache write/owner fallback, full-page mint/burn dispatch inertness, holder count/loading/refresh/scan display, stats-section descriptor wiring, mini-card button/status descriptors, passive admin/action disabled states, mint-input shell routing, holder/cache fallback, cache-revision reload lifecycle, forced-count refresh option planning, and already-tried event-scan suppression are pinned. | Realtime listeners, localStorage/timer lifecycle, contract reads/writes, mint/burn/password/invite execution, metadata fetch/write, refresh dispatch, cache rereads/writes, route, wallet/provider, and state mutation. | Realtime refresh and cache hydration are still coupled to contract, cache reread, and timer lifecycles; only the central refresh decision is pure-plan routed. | Next safe lane: continue cache-revision/realtime lifecycle characterization only where pure plans delete parent branch logic; do not move listener, contract, wallet, metadata, cache-write, timer, refresh dispatch, cache reread, or state execution. |
