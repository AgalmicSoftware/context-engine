...
- Current lengths:
  ...
  - `surveyResultsAnalysisLifecyclePlan.ts`: **170 lines**
...
|---|---|---|---|---|---|---|
| SurveyResults cache write boundaries | Filter bookmark, survey/question bookmark, selected fallback write, and analysis artifact write dispatch use narrow injected-port controllers. | `surveyResultsCacheWriteEligibilityPlan.ts` defines target identities, blocked reasons, payload descriptors, state-patch descriptors, and outcome shapes from passed-in values; `surveyResultsAnalysisLifecyclePlan.ts` defines analysis artifact target identity, payload descriptors, blocked reasons, lifecycle status/result, and failure-recovery state patches from passed-in values. | Identity/payload matrices, write-readiness matrix, selected fallback/status coverage, and analysis artifact success, blocked/no-op, partial failure/retry, parent status output, stale artifact regeneration, missing identity no-op, plan-derived failure state patch, and no-download/fetch/decrypt side effects are pinned. | Cache reads, broad cache persistence, cache normalization, feedback timers, warning/error logging, AI generation, artifact merge, export/download execution, fetch, decrypt, route, polling, and state application. | Broad persistence still crosses cache reads, polling/status state, AI/export artifacts, artifact merge, and parent feedback lifecycles. Analysis artifact lifecycle planning is pure and extracted, but broad persistence is still parent-owned. | Next safe lane: SurveyResults analysis artifact lifecycle is the first candidate for a meatier move, but only as a pre-controller around section-generation status/ordering with injected ports; keep cache reads, AI generation, artifact merge, export/download, polling, and state application parent-owned unless separately tested. |
