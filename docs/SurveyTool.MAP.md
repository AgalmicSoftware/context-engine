...
- Current lengths:
  ...
  - `SurveyQuestions.tsx`: **8,379 lines**
  - `SurveyQuestionsRouteSurface.tsx`: **79 lines**
  - `SurveyQuestionsTopRouteSection.tsx`: **48 lines**
  - `SurveyQuestionsRouteBodySection.tsx`: **33 lines**
  - `SurveyQuestionsAuthoringRouteSection.tsx`: **57 lines**
  - `SurveyQuestionsResponseRouteSection.tsx`: **41 lines**
  - `SurveyQuestionsJsonRouteSection.tsx`: **46 lines**
  - `SurveyQuestionsTagModalSlot.tsx`: **25 lines**
  - `surveyQuestionsRouteSurfaceTypes.ts`: **161 lines**
  - `surveyQuestionsRouteJsonControlsProps.ts`: **56 lines**
  ...
  - `SurveyQuestionsFullQuestionGatedPromptCard.tsx`: **32 lines**
  - `SurveyQuestionsFullQuestionResponseInput.tsx`: **187 lines**
  - `SurveyQuestionsFullQuestionSliderSection.tsx`: **102 lines**
  - `SurveyQuestionsJsonControls.tsx`: **138 lines**
  - `SurveyQuestionsJsonTree.tsx`: **121 lines**
  - `SurveyQuestionsResponseView.tsx`: **127 lines**
  - `SurveyQuestionsSubmittedResponseView.tsx`: **66 lines**
  - `SurveyQuestionsSubmitFooter.tsx`: **146 lines**
  - `SurveyQuestionsSurveyAnswersView.tsx`: **56 lines**
  - `SurveyQuestionsTopStrip.tsx`: **90 lines**
  - `SurveyQuestionsUserResponseNotice.tsx`: **86 lines**
  - `surveyQuestionsFullQuestionResponseInputState.ts`: **229 lines**
  ...
  - `SurveyResultsDisplayPanels.tsx`: **183 lines**
  - `SurveyResultsFilterSummary.tsx`: **47 lines**
  ...
  - `SurveyResultsQuestionTable.tsx`: **93 lines**
  - `SurveyResultsQuestionTableRow.tsx`: **74 lines**
  - `SurveyResultsModalHeader.tsx`: **137 lines**
  - `SurveyResultsPanels.tsx`: **80 lines**
  - `SurveyResultsSyncDetailsDisplay.tsx`: **128 lines**
  ...
  - `SurveyResultsIndividualResponsesList.tsx`: **49 lines**
  - `SurveyResultsIndividualResponseCard.tsx`: **73 lines**
  - `SurveyResultsIndividualResponseBody.tsx`: **143 lines**
  ...
  - `surveyResultsCacheReadinessDisplayPlan.ts`: **114 lines**
  - `surveyResultsCacheControllerSnapshot.ts`: **212 lines**
  - `surveyResultsManualRefreshController.ts`: **67 lines**
  - `surveyResultsQueuedRefreshController.ts`: **47 lines**
  - `surveyResultsFallbackQuestionHelpers.ts`: **93 lines**
  - `surveyResultsFallbackQuestionWriteController.ts`: **66 lines**
  - `surveyResultsQuestionMetadataReadController.ts`: **86 lines**
  - `surveyResultsQuestionNetworkReadController.ts`: **367 lines**
  - `surveyResultsCacheWriteEligibilityPlan.ts`: **288 lines**
  - `surveyResultsAnalysisLifecyclePlan.ts`: **239 lines**
  - `surveyResultsAnalysisLifecycleController.ts`: **113 lines**
  - `surveyResultsAnalysisArtifactCachePorts.ts`: **155 lines**
  - `surveyResultsAnalysisArtifactReadController.ts`: **96 lines**
  - `surveyResultsAnalysisGeneratedArtifactCompletionPlan.ts`: **182 lines**
  ...
  - `surveyResultsHelpers.ts`: **739 lines**
  - `SurveyPileViewMode.tsx`: **2,780 lines**
  - `surveyQuestionsJsonDerivation.ts`: **131 lines**
- Summary: the runtime is no longer one monolithic file, but `SurveyQuestions.tsx` is still the dominant shared state machine. `SurveyQuestions.tsx` now has a presentational route surface for loading/top-strip/answer-vs-authoring/JSON/tag-modal composition with centralized route-surface prop contracts, route JSON-controls prop assembly, passive JSON tree, submitted-response, survey-answer, submit-footer, full-question slider, full-question response input display/action descriptors, full-question content/display sections, gated-prompt card chrome, primary-submit decision dispatch, submit-readiness descriptors, submitted-response URL planning, submit-start/status sequencing, stale-submit cleanup, submit completion/status handoff, single-question source-restore context planning, single-question cache-bootstrap flow planning, cache-bootstrap stop-handling planning, seeded response hydration patch planning, current-question preservation planning, and question/survey decrypt planning extracted while keeping submit execution, real decrypt execution, JSON generation/copy execution, cache reads/writes, fetches, retry scheduling, live route/navigation state, and mutation behavior in the parent. `SurveyResults.tsx` now has a presentational route/report surface, passive display panels, demo alternate-results surface display, selected-question summary display assembly, pure summary view-models, freeform/multichoice summary display rows, export orchestration, filter/export controls, HTML report modal prop assembly, HTML report export modal display/action label planning with explicit display/execution prop contracts, modal display descriptors, export filename descriptors, section availability table display, analysis action display, filter/status display-plan, typed sync-status display-plan handoff to the sync panel and sync-details display, export display and HTML report readiness planning, HTML/PDF report download request/execution-plan identity and settlement patch planning, demo analysis artifact composition, cache/readiness display planning, a cache-controller input snapshot, narrow manual-refresh and queued-refresh dispatch controllers, selected-question fallback planning plus injected fallback/status write dispatch, read-only selected-question metadata and scoped question-network cache controllers, filter-bookmark, survey/question bookmark, analysis artifact cache-write eligibility/pre-readiness planning, typed analysis artifact cache target/key/read request/selection port planning, a narrow injected-port analysis artifact read controller, a typed pure generated-artifact completion plan, a typed pure analysis persistence lifecycle plan, a narrow injected-port analysis lifecycle status/order controller, injected bookmark/artifact write dispatch, refresh/status sequence and write planning, and question-summary status helpers extracted while keeping fetch, decrypt execution, non-analysis concrete cache reads, cache API selection/port provisioning, broad cache writes/persistence, analysis read generation fallback, route, mutation behavior, polling/timers, latest-block lookup, manual-refresh target state/polling follow-up, queued-refresh dispatch, refresh/status state application ports, AI generation, artifact merge, generated-artifact completion execution, export payload generation, report rendering, DOM/PDF capture, and download execution in the parent. Analysis artifact persistence now has parent-level lifecycle coverage for success, cached-ready final state cleanup, blocked/no-op eligibility, partial failure/retry recovery, status output, stale artifact regeneration, stale cached artifact rejection, no-artifact no-read persistence skips, typed cache read args, read exception fallback, partial cached payload rejection, generated completion identity/cache-write/lifecycle descriptors, report download identity/settlement descriptors, and no-download side effects.
...
| `surveyQuestionsTypes.ts` | Shared state/display/readiness builders | Defines SurveyQuestions state patches, display state builders, primary-submit plans, and submit-readiness descriptors from passed-in values while leaving submit execution, route mutation, cache/decrypt/fetch/worker behavior, and state application in `SurveyQuestions` |
...
| `SurveyResultsDisplayPanels.tsx` | Results display panel ordering | Renders the status, view-mode toggle, locked banner placement, and delegates filter summary, individual response list/body, question-list, and summary-list display panels from explicit props while leaving fetch, decrypt execution, filter state, route/session state, export/download execution, cache, polling, and mutation behavior in `SurveyResults` |
...
| `surveyResultsManualRefreshController.ts` | Manual-refresh dispatch decision | Runs only the injected refresh port decision for question vs survey mode while leaving latest-block lookup, target block state, polling/backoff follow-up, cache reads/writes, fetch, decrypt, route, and export behavior in `SurveyResults` |
...
| `surveyResultsQuestionNetworkReadController.ts` | Scoped question-network cache read status | Reads sync or async scoped question buckets through injected read-only ports, merges ready question/response network data, and returns a plain result/memo patch while leaving concrete cache API selection, cache writes/persistence, fetch, decrypt, export, route, polling, timers, and state application in `SurveyResults` |
| `surveyResultsCacheWriteEligibilityPlan.ts` | Cache-write eligibility and pre-readiness planning | Derives parent-owned filter, survey/question bookmark, and analysis artifact write targets, pre-read cache readiness, blocked reasons, payload shapes, invalid-cache warning flags, success-feedback decisions, and state-patch descriptors from passed-in values while leaving cache reads, cache writes/persistence, timers, feedback/state application, AI generation, fetch, decrypt, export, route, polling, and mutation behavior in `SurveyResults` |
| `surveyResultsAnalysisLifecyclePlan.ts` | Analysis artifact lifecycle planning | Chooses current vs cached analysis artifacts, requested/covered/missing report sections, target identity, payload descriptors, blocked reasons, lifecycle status/result, and failure-recovery state patches from passed-in values while leaving cache reads, cache writes/persistence, AI generation, artifact merge, export/download execution, route, polling, timers, and state application in `SurveyResults` |
...
| `SurveyResultsIndividualResponseBody.tsx` | Individual response body presentation | Builds individual response display rows, applies parent-provided decrypted override values for display, and renders `SingleQuestionResponse` cards while leaving locked-response decrypt execution, override state, cache, polling, export, route/session state, and mutation behavior in `SurveyResults` |
...
|---|---|---|---|---|---|---|
| SurveyResults cache and report boundaries | Filter bookmark, survey/question bookmark, selected fallback write, analysis artifact cache-read dispatch, analysis artifact cache-write dispatch, and analysis lifecycle ready/blocked/generation-start/failure-recovery status dispatch use narrow injected-port controllers. HTML/PDF report download identity, ready/blocked execution plans, and settlement patches are pure-descriptor routed while render/download execution and state application stay parent-owned. | `surveyResultsCacheWriteEligibilityPlan.ts` defines target identities, blocked reasons, payload descriptors, state-patch descriptors, analysis artifact pre-read readiness, and outcome shapes from passed-in values; `surveyResultsAnalysisArtifactCachePorts.ts` defines typed analysis-cache read/write port signatures, cache-target normalization, cache-key derivation, sync read request descriptors, and selected-artifact extraction; `surveyResultsAnalysisArtifactReadController.ts` defines the typed read/skipped/failed result for already-built analysis cache read requests; `surveyResultsAnalysisGeneratedArtifactCompletionPlan.ts` defines generated artifact usability, cache-write payload descriptors, lifecycle success patch descriptors, stale/missing failure shape, and missing-section metadata from passed-in values; `surveyResultsAnalysisLifecyclePlan.ts` defines analysis artifact target identity, payload descriptors, blocked reasons, lifecycle status/result, cached-ready cleanup, and failure-recovery state patches from passed-in values; `surveyResultsAnalysisLifecycleController.ts` defines the typed status/order dispatch result for already-built lifecycle plans; `surveyResultsExportDisplayHelpers.ts` defines HTML report blocked/ready download attempt and settlement patch descriptors; `surveyResultsHtmlReportDownloadRequest.ts` defines the report export kind, filename identity, render-option descriptor, and ready/blocked execution-plan descriptor. | Identity/payload matrices, write-readiness matrix, selected fallback/status coverage, broad-normalization blocker classification, analysis lifecycle controller fake-port routing, analysis read controller fake-port routing, generated completion plan identity/cache-write/lifecycle descriptors, HTML/PDF report download filename/render-option/execution-plan identity, report blocked/ready and success/failure settlement descriptors, and analysis artifact success, cached-ready cleanup, blocked/no-op, partial failure/retry, parent status output, stale artifact regeneration, stale cached artifact rejection, typed cache read args, read exception fallback, partial cached payload rejection, no-artifact no-read persistence skips, missing identity no-op, plan-derived failure state patch, and no-download/fetch/decrypt side effects are pinned. | Non-analysis concrete cache reads, cache API selection/port provisioning, concrete cache writes, broad cache persistence, cache normalization, feedback timers, warning/error logging, AI generation, artifact merge execution, generated-artifact completion execution, report rendering, DOM/PDF capture, download execution, fetch, decrypt, route, polling, generation fallback, and concrete state application ports. | Broad persistence still crosses concrete cache writes, non-analysis cache reads, cache API selection, polling/status state, AI/export artifacts, artifact merge execution, generated-artifact completion execution, and parent feedback lifecycles. Analysis artifact cache read dispatch, analysis artifact cache write dispatch, generated completion planning, analysis artifact cache port/request shapes, analysis artifact pre-read readiness, analysis lifecycle status/order, report download identity/execution-plan descriptors, and report settlement patch descriptors are controller/pure-plan routed; broad results cache normalization is test-pinned only, and HTML report execution is still blocked by report rendering, DOM/PDF capture, file download, and parent `setState` ownership. | Next safe lane: keep broad cache persistence and HTML/PDF export execution parent-owned; generated-artifact completion should stay pure-plan only until a dispatcher can be proven without owning cache-write execution, AI generation, artifact merge execution, export/download, polling, route/session, decrypt, or concrete state application. |
| SurveyResults refresh/status and read-only cache orchestration | Manual refresh, queued refresh, selected metadata reads, scoped network reads, and fallback write dispatch have narrow helper/controller boundaries. | `surveyResultsHelpers.ts`, `surveyResultsCacheControllerSnapshot.ts`, `surveyResultsQuestionMetadataReadController.ts`, and `surveyResultsQuestionNetworkReadController.ts` hold pure/read-only contract shapes. | Refresh/status no-op/failure sequencing, readiness display, metadata read, scoped network read, and parent shell wiring are pinned. | Latest-block network reads, setState application, polling/backoff follow-up, queued reason collection, concrete cache API selection, fetch, decrypt, export, route, and timers. | Polling and state sequencing are still one lifecycle; moving execution would require broad lifecycle ports. | Keep pure-plan/read-only only until a single lifecycle decision can be tested without owning polling or state application. |
| SurveyQuestions response input and submit readiness | Full-question response input display/action descriptors and submit-readiness descriptors are pure/helper-routed. | `surveyQuestionsFullQuestionResponseInputState.ts` defines question-type input state and answer/rating/encryption action identity; `surveyQuestionsTypes.ts` defines submit-readiness status from pending stats, submitting state, and masked current-question state. | Disabled multichoice/binary/rating no-dispatch, audio worker identity, callback argument order, rating commit identity, encryption-toggle inertness while submitting, encrypted upload readiness, masked single-question submit blocking, and submit no-dispatch plans are pinned. | Handler execution, setState, draft persistence, submit/encrypt/decrypt/Lit execution, cache reads/writes, fetch/retry/timers, route/session mutation, worker/audio execution, and telemetry remain parent-owned. | Further extraction would mostly package callbacks or create a generic renderer without deleting parent execution branches. | Next safe lane: only extract a direct question-type or response-action descriptor when it removes parent branching and keeps execution parent-owned; otherwise move to SurveyQuestions final decomposition review. |
...
| SBTPage/SBTsList realtime/cache readiness | Full-page mint/burn action rendering now sits in a passive action surface fed by a pure full-action display plan and named execution props; holder count/refresh/scan-progress presentation now sits in `SbtPageHolderStatusDisplay.tsx` fed by typed holder-count and scan-progress descriptors; mini-card display state, admin action display planning, mint-input display helpers, central holder-refresh lifecycle planning, cache-revision reload planning, load-rerun queue planning, SBT list realtime-progress bridge planning, session-chip progress visibility/display planning, SBT list initial/chip loading-status collection planning, SBT list section loading-readiness/display planning, and SBT list section body chrome are extracted, but realtime/cache execution is not controller-routed. | SBT helper modules define holder/action/status/session/mini-card readiness plans, the full-page action display plan, the admin action display plan, pure central-refresh lifecycle plan, cache-revision reload plan, pending load-rerun queue plan, `SBTsList` realtime progress input/retention plans, session-chip progress desired-visibility/sticky transition/display plans, initial/chip loading-status collection plans, section discovery/search/spinner/refresh-busy readiness plans, and section loading/empty display plans from passed-in values; `SBTsList.tsx` is 3254 lines after delegating status/readiness display collection and section body chrome while keeping the per-session snapshot resolver and timer/state execution parent-owned; `SbtPageFullActionButtons.tsx` consumes the full-action descriptors and routes only explicit mint/burn props through existing controllers; `SbtPageAdminActions.tsx` consumes admin display plans and named execution props; `SbtPageHolderStatusDisplay.tsx` defines the passive holder count and scan-progress display contracts consumed by `SbtPageStatsSection.tsx`; `sbtListRealtimeProgressHelpers.ts` describes live-progress bridge updates, stale-prune decisions, and next-prune timing without owning refs or timers; `sbtListChipProgressVisibilityHelpers.ts` describes chip progress text/width/style display decisions plus visibility initialization, pending-sync, delayed commit, timer-retention, and stale-meta removal decisions without owning refs, timers, render callbacks, or state updates. | Complete cached-metadata readiness without refresh/direct metadata/cache write/owner fallback, full-page mint/burn display-plan identity and dispatch inertness, admin action display-plan identity/inertness, holder count/loading/refresh/scan display, stats-section descriptor wiring, mini-card button/status descriptors, passive admin/action disabled states, mint-input shell routing, holder/cache fallback, cache-revision reload lifecycle, load-rerun queue merging, forced-count refresh option planning, realtime progress bridge input/retention planning, session-chip visibility/display transition planning, initial loader fallback/status row planning, chip loading-status selected-scope filtering, section discovery/search/spinner/refresh-busy readiness planning, section body content/loading/empty display, and already-tried event-scan suppression are pinned. | Realtime listeners, recent-progress ref mutation, localStorage/timer lifecycle, contract reads/writes, mint/burn/password/invite execution, metadata fetch/write, refresh dispatch, cache rereads/writes, load-rerun scheduling, route, wallet/provider, and state mutation. | Realtime refresh, cache hydration, ref mutation, timeout scheduling, and rerun scheduling are still coupled to contract, cache reread, and timer lifecycles; only the central refresh, cache-revision reload, pending-rerun queue, SBT list realtime-progress bridge, session-chip progress visibility/display, SBT list loading-status collection, SBT list section-readiness/display decisions, and section body chrome are pure/display routed. | Next safe lane: leave SBT list unless a pure session-selector display model deletes parent branch logic; do not move listener, contract, wallet, metadata, cache-write, timer, refresh dispatch, cache reread, load-rerun scheduling, ref mutation, or state execution. |
| SessionWizard publish readiness/controller boundary | Publish step sequencing, register-step tx/status, completion callbacks, and worker auto-deploy slices use injected controllers; publish readiness, metadata fallback selection, execution-step display, progress display, and publish-section shell visibility now sit behind explicit pure/display boundaries. | `sessionWizardPublishReadiness.ts`, `sessionWizardPublishController.ts`, `sessionWizardNormalModeCards.ts`, `SessionWizardPublishSection.tsx`, and publish summary props define explicit readiness/status/display contracts. | Worker/storage readiness, manual/uploaded metadata fallback, blank manual metadata blocked/inert state, custom-worker blocked publish state, manual controls, publish progress, pure UI execution-step planning, publish-section visibility/callback wiring, and SBT finalization wiring are pinned. | Metadata upload, worker deploy execution, storage/Arweave writes, SBT deploy/finalization execution, wallet/provider/contract calls, route mutation, and state mutation. | Publish still crosses worker deploy, storage, registry write, pending SBT finalization, and wallet/contract lifecycles. | Keep publish readiness/progress planning pure; a future move must isolate one injected-port decision without owning upload, worker, storage, contract, or route execution. |
