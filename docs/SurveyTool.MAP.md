...
- Current lengths:
  ...
  - `SurveyQuestions.tsx`: **8,851 lines**
  ...
  - `surveyToolSingleQuestionCacheBootstrapController.ts`: **296 lines**
...
| `surveyToolSingleQuestionCacheBootstrapController.ts` | Single-question cache-bootstrap planning | Resolves cache/recent-payload bootstrap status, pure parent flow plans, and seeded response hydration patch shape while leaving cache writes, metadata fetches, response fetches, retry scheduling, route/session state, and state application in `SurveyQuestions` |
