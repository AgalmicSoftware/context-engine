...
| `surveyResultsLifecycleRuntime.ts` | Results component-update lifecycle runtime | Runs component-update patch queuing, modal open/close URL decisions, cache-ready refresh reasons, view/survey reset dispatch, nonce tick dispatch, and question-scope reset dispatch through injected ports while leaving mount/unmount setup, concrete `setState`, refs, polling runtimes, and parent callback implementations in `SurveyResults` |
