...
|---|---|---|---|---|---|---|
| SurveyResults cache write boundaries | Filter bookmark, survey/question bookmark, selected fallback write, and analysis artifact write dispatch use narrow injected-port controllers. | `surveyResultsCacheWriteEligibilityPlan.ts` defines target identities, blocked reasons, payload descriptors, state-patch descriptors, and outcome shapes from passed-in values. | Identity/payload matrices, write-readiness matrix, selected fallback/status coverage, and analysis artifact success, blocked/no-op, failure, retry, parent status output, and no-download side effects are pinned. | Cache reads, broad cache persistence, cache normalization, feedback timers, warning/error logging, AI generation, artifact merge, export/download execution, fetch, decrypt, route, polling, and state application. | Broad persistence still crosses cache reads, polling/status state, AI/export artifacts, and parent feedback lifecycles. Analysis artifact persistence is covered enough for a future tiny lifecycle-decision follow-up, but not for moving broad persistence. | Next safe lane: keep broad cache persistence parent-owned; if more work is needed, add one pure analysis persistence lifecycle decision or one test-only follow-up without moving cache reads, AI generation, artifact merge, export/download, polling, or state application. |
