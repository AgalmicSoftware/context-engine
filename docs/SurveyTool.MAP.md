...
| `surveyResultsCacheWriteEligibilityPlan.ts` | Bookmark cache-write eligibility planning | Derives parent-owned filter and survey/question bookmark write targets, blocked reasons, payload shapes, invalid-cache warning flags, success-feedback decisions, and state-patch descriptors from passed-in values while leaving cache reads, cache writes/persistence, timers, feedback/state application, fetch, decrypt, export, route, polling, and mutation behavior in `SurveyResults` |
...
| `surveyResultsHelpers.ts` | Shared SurveyResults pure helpers | Includes selected fallback/status helpers and the refresh/status write plan, including latest-block state-patch planning from parent-provided values while leaving latest-block reads, setState application, polling follow-up, cache reads/writes/persistence, fetch, decrypt, route, export, and timers in `SurveyResults` |
...
| SurveyResults refresh/status write readiness | Refresh/status writes now have eligibility, failure/recovery coverage, and a pure latest-block state-patch plan used by nonce and manual refresh paths. Parent still owns latest-block network reads, setState application, polling/backoff follow-up, queued refreshes, and refresh dispatch. | Keep refresh/status pure-plan-only until a controller can preserve parent-owned polling/state sequencing without broad lifecycle ports. |
| SurveyResults remaining write blockers | Report-analysis/export artifact cache writes remain AI/export-adjacent and too broad for this lane. Broader cache persistence, normalization, and polling-coupled refresh orchestration are still deferred. | Next lane should start with report-analysis/export artifact write tests or a refresh/status sequencing design, and stop before a controller unless identity, payload, failure/recovery, and lifecycle ownership are pinned. |
