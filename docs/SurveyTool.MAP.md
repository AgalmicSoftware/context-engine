...
|---|---|---|---|---|---|---|
| SurveyResults cache write boundaries | Filter bookmark, survey/question bookmark, selected fallback write, analysis artifact cache-write dispatch, and analysis lifecycle ready/blocked/generation-start/failure-recovery status dispatch use narrow injected-port controllers. | `surveyResultsCacheWriteEligibilityPlan.ts` defines target identities, blocked reasons, payload descriptors, state-patch descriptors, analysis artifact pre-read readiness, and outcome shapes from passed-in values; `surveyResultsAnalysisArtifactCachePorts.ts` defines typed analysis-cache read/write port signatures, cache-key derivation, sync read request descriptors, and selected-artifact extraction; `surveyResultsAnalysisLifecyclePlan.ts` defines analysis artifact target identity, payload descriptors, blocked reasons, lifecycle status/result, cached-ready cleanup, and failure-recovery state patches from passed-in values; `surveyResultsAnalysisLifecycleController.ts` defines the typed status/order dispatch result for already-built lifecycle plans. | Identity/payload matrices, write-readiness matrix, selected fallback/status coverage, broad-normalization blocker classification, analysis lifecycle controller fake-port routing, and analysis artifact success, cached-ready cleanup, blocked/no-op, partial failure/retry, parent status output, stale artifact regeneration, stale cached artifact rejection, typed cache read args, partial cached payload rejection, no-artifact no-read persistence skips, missing identity no-op, plan-derived failure state patch, and no-download/fetch/decrypt side effects are pinned. | Concrete cache reads/writes, broad cache persistence, cache normalization, feedback timers, warning/error logging, AI generation, artifact merge, export/download execution, fetch, decrypt, route, polling, and concrete state application ports. | Broad persistence still crosses concrete cache reads/writes, polling/status state, AI/export artifacts, artifact merge, and parent feedback lifecycles. Analysis artifact cache writes, analysis artifact cache port/request shapes, analysis artifact pre-read readiness, and analysis lifecycle status/order are controller/pure-plan routed, broad results cache normalization is test-pinned only, and HTML report export/download persistence is blocked by DOM download and payload generation ownership. | Next safe lane: keep broad cache persistence parent-owned; the next meatier move is a narrow analysis cache execution pre-controller only if it can keep concrete cache reads/writes, AI generation, artifact merge, export/download, polling, and concrete state application parent-owned. |
