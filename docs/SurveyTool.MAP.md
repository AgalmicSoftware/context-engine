...
SurveyTool.tsx  [top-level wrapper]
  -> SurveySelector.tsx  [survey/questions selector + filter + results]
     ...
     -> SurveyResults.tsx  [survey/question results runtime]
        ...
        -> surveyResultsCacheWriteEligibilityPlan.ts  [filter and survey/question bookmark cache-write eligibility plans]
...
## Frontend Modernization Readiness Matrix

| Area | Status | Next safe lane |
|---|---|---|
| SurveyResults filter bookmark write controller | The first narrow filter-bookmark write dispatch runs through an injected-port controller after parent-owned eligibility planning. Parent still owns filter cache reads, warning/error logging, success feedback state/timer, and all broad persistence. | Keep this controller narrow; require same-stage recovery/regression coverage before expanding only within the same filter-bookmark boundary. |
| SurveyResults survey/question bookmark write controller | The second narrow bookmark write path now has behavior coverage, pure eligibility planning, and injected write dispatch for survey/question bookmark cache writes. Parent still owns bookmark cache reads, state application, error logging, and all broad persistence. | Treat this lane as complete; do not broaden it into selected-result/status, refresh/status, report-analysis, decrypt, export, polling, or cache persistence behavior. |
| SurveyResults selected-result/status write controller | Selected fallback/status writes now have behavior coverage, pure planning, direct controller coverage, parent wiring coverage, and injected fallback write dispatch. Parent still owns selected identity derivation, fallback memo bucket ownership, state application, cache reads/persistence, fetch, decrypt, export, route, polling, and timers. | Treat this as a narrow complete lane; do not broaden it into persistent cache writes, refresh/status state application, report-analysis, decrypt, export, polling, or cache persistence behavior. |
| SurveyResults refresh/status write readiness | Refresh/status writes now have eligibility, happy-path/no-op/failure sequencing coverage, and a pure sequence/write plan used by nonce and manual refresh paths. The plan derives ordered effect descriptors and state patches only. Parent still owns latest-block network reads, setState application, polling/backoff follow-up, queued refresh dispatch, and refresh dispatch. | Keep refresh/status pure-plan-only until a controller can preserve parent-owned polling/state sequencing without broad lifecycle ports. |
| SurveyResults analysis/export artifact write controller | Report-analysis/export artifact cache writes now have method-boundary coverage, pure target/payload planning, direct controller coverage, parent failure/recovery status coverage, and injected write dispatch. Parent still owns cache reads, write error propagation, AI generation flow, artifact normalization/merge, export payload generation, and download execution. | Treat this as a narrow complete lane; do not broaden it into AI generation, export/download execution, broad cache persistence, polling, decrypt, or route behavior. |
| SurveyResults remaining write blockers | Broader cache persistence, normalization, and polling-coupled refresh orchestration are still deferred. The cache-write eligibility helper now has explicit identity and payload-shape matrix coverage for analysis artifacts, filter bookmarks, and survey/question bookmarks, including missing route/cache identifiers and current parent-owned missing bookmark-id behavior. | Next lane should stop before any controller unless failure/recovery and lifecycle ownership are pinned at the same boundary; broad persistence and validation remain parent-owned blockers. |
| SurveyResults read-only orchestration status | Selected-question metadata, scoped question-network sync/async reads, fallback question planning, readiness display, and cache-controller snapshots are extracted behind read-only helpers/controllers. No useful consolidation candidate currently deletes parent code without adding indirection. | Continue only where a read-only move deletes meaningful parent orchestration or removes duplicated planning. |
| SurveyQuestions decrypt/cache readiness | Encrypted submitted JSON readiness, same-envelope decrypted-value restore across cache refresh, refreshed-plaintext precedence over stale decrypted values, and recent single-question payload restore before response bootstrap are covered. Real decrypt execution, submit execution, worker/storage, route, wallet/provider, and cache writes remain parent-owned. | Add targeted tests before moving any decrypt/cache source restoration decision; do not move Lit calls or submit/cache execution. |
| UserPage cache/decrypt readiness | Cache-derived decrypt eligibility, gated cached response visibility, encrypted cache refresh that stays masked without decrypt execution, and cached analysis visibility with refresh disabled during active analysis are covered at the parent/display boundary. Real decrypt execution, cache writes, bookmark persistence, AI fetch, route mutation, and refresh execution remain in `UserPage`. | Next safe move is another pure display/eligibility plan only if it deletes parent decision code; otherwise prefer tests. |
| SBTPage cache/realtime readiness | Passive admin action disabled-state inertness is now pinned at the display boundary. The remaining shell still mixes cache, realtime/localStorage timers, contract reads, password/invite mint flows, and display handoff. | Tests first around one passive cache/realtime status boundary; avoid moving contract, storage, Lit, timer, or route behavior. |
| SessionWizard publish/controller readiness | Publish summary worker/source display, manual metadata callback routing, active metadata-source display, and manual gas override callback routing are now pinned without publish execution. Publish/controller extraction remains separate from SurveyResults cache-write work. | Tests first around one passive publish/readiness decision; avoid moving session mutation, route mutation, contract, worker, storage, or upload execution. |
