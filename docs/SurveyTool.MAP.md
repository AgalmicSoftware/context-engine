...
- Current lengths:
  ...
  - `SurveyResultsHtmlReportExportModal.tsx`: **327 lines**
...
| `SurveyResultsHtmlReportExportModal.tsx` | HTML report export modal presentation | Builds pure action-label/session/exporter display decisions, renders export-format selection, section availability, demo-mode toggle, analysis-generation affordance, and delegates close/download controls to `SurveyResultsHtmlReportActionControls.tsx` from explicit display props and named execution callback props while leaving snapshot construction, AI generation, export rendering, and browser download execution in `SurveyResults` |
