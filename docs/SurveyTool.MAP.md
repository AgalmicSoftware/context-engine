...
- Current lengths:
  ...
  - `SurveyQuestions.tsx`: **8,840 lines**
  ...
  - `surveyToolSingleQuestionCacheBootstrapController.ts`: **418 lines**
  - `surveyToolDecryptFlow.js`: **2,015 lines**
  - `SurveyResults.tsx`: **5,327 lines**
  ...
  - `surveyResultsExportDisplayHelpers.ts`: **346 lines**
  ...
  - `surveyResultsAnalysisGeneratedArtifactCompletionPlan.ts`: **177 lines**
...
SurveyTool.tsx  [top-level wrapper]
  -> SurveySelector.tsx  [survey/questions selector + filter + results]
     ...
     -> SurveyQuestions.tsx  [shared full response runtime]
        ...
        -> surveyToolSingleQuestionCacheBootstrapController.ts  [single-question cache-bootstrap flow and seeded hydration planning]
     ...
     -> SurveyResults.tsx  [survey/question results runtime]
        ...
        -> surveyResultsAnalysisArtifactCachePorts.ts  [analysis artifact cache key/read request/selection port contracts]
        ...
        -> surveyResultsExportDisplayHelpers.ts  [export labels/plans and demo analysis artifact composition]
...
| `surveyToolSingleQuestionCacheBootstrapController.ts` | Single-question cache-bootstrap planning | Resolves cache/recent-payload bootstrap status, pure parent flow plans, stop-handling descriptors, and seeded response hydration patch shape while leaving cache writes, metadata fetches, response fetches, retry scheduling, route/session state, and state application in `SurveyQuestions` |
...
| `surveyResultsAnalysisArtifactCachePorts.ts` | Analysis artifact cache port contracts and read request planning | Defines typed analysis-cache read/write port signatures, cache-key derivation, sync read request descriptors, and selected-artifact extraction from passed-in cache values while leaving `peekCacheSync`, `readCache`, `writeCache`, AI generation, artifact merge, export/download execution, polling, route, decrypt, and state application in `SurveyResults` |
...
| SurveyQuestions decrypt/cache readiness | Submit-status handoff, single-question cache-bootstrap flow decisions, cache-bootstrap stop-handling descriptors, seeded hydration patch planning, and question-decrypt status planning have extracted pure/controller boundaries, but decrypt/cache restore execution is not controller-routed. | `surveyToolSingleQuestionCacheBootstrapController.ts`, `surveyToolResponseSourceController.ts`, and `surveyQuestionsJsonDerivation.ts` define cache-bootstrap/source/JSON-readiness decisions from passed-in values. | Encrypted submitted JSON readiness, same-envelope restore, refreshed-plaintext precedence, recent payload restore, stale recent-payload fallback to normal metadata loading, single-question cache-bootstrap flow/stop-handling planning, seeded hydration patch planning, and disabled local-cache fallback are pinned. | Lit/decrypt execution, submit execution, worker/storage, cache writes, route/navigation, retry scheduling, wallet/provider, and state mutation. | Source restore still meets real decrypt/cache/submit lifecycles inside the parent. | Add only source-restore helper extraction if it reduces duplicated decision logic; do not move Lit calls or cache/submit execution. |
| UserPage cache/decrypt/action boundaries | Analyze, bookmark, cache-refresh clicks, cache refresh state patch planning, and deep-scan cache-refresh carry/loading input composition use injected or pure-plan helpers. | `userPageActionController.ts`, `userPageLoadingStateHelpers.ts`, and UserPage helper modules define action/readiness/loading/cache-refresh patch contracts from passed-in values. | Cached responses, gated visibility, no-account decrypt-click inertness, decrypt-click wiring, stale/refresh boundaries, incomplete cache lane AI/compare disabling, visible gated cached data, cache refresh state patch planning, deep-scan refresh carry/loading input planning, and cached-analysis refresh boundaries are pinned. | Real decrypt execution, cache writes, bookmark persistence, AI fetch, route mutation, timers, refresh execution, telemetry emission, and state application. | Cache/decrypt refresh still crosses user visibility, fetch, analysis, and parent state application. | Next safe lane: keep refresh execution parent-owned; only extract pure patch/display plans that delete parent decision code without forwarding broad prop bundles. |
| SBTPage realtime/cache readiness | Some action/status and mint-input display helpers are extracted, but realtime/cache execution is not controller-routed. | SBT helper modules define holder/action/status/session readiness plans from passed-in values. | Complete cached-metadata readiness without refresh/direct metadata/cache write/owner fallback, passive admin/action disabled states, mint-input shell routing, holder/cache fallback, and cache-revision reload lifecycle are pinned. | Realtime listeners, localStorage/timer lifecycle, contract reads/writes, mint/burn/password/invite flows, metadata fetch/write, route, wallet/provider, and state mutation. | Realtime refresh and cache hydration are still coupled to contract and timer lifecycles. | Tests first around one passive cache/realtime status boundary; do not move listener, contract, wallet, or metadata execution. |
| SessionWizard publish readiness/controller boundary | Publish step sequencing, register-step tx/status, completion callbacks, and worker auto-deploy slices use injected controllers; publish readiness remains a pure helper. | `sessionWizardPublishReadiness.ts`, `sessionWizardPublishController.ts`, `sessionWizardNormalModeCards.ts`, and publish summary props define explicit readiness/status/display contracts. | Worker/storage readiness, manual/uploaded metadata fallback, blank manual metadata blocked/inert state, custom-worker blocked publish state, manual controls, publish progress, and SBT finalization wiring are pinned. | Metadata upload, worker deploy execution, storage/Arweave writes, SBT deploy/finalization execution, wallet/provider/contract calls, route mutation, and state mutation. | Publish still crosses worker deploy, storage, registry write, pending SBT finalization, and wallet/contract lifecycles. | Keep publish readiness pure; a future move must isolate one injected-port decision without owning upload, worker, storage, contract, or route execution. |
