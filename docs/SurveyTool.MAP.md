...
|---|---|---|---|---|---|---|
| SurveyResults cache and report boundaries | Concrete results cache access, filter bookmark, survey/question bookmark, selected fallback write, analysis artifact cache-read dispatch, analysis artifact cache-write dispatch, analysis artifact merge/normalize, and analysis lifecycle ready/blocked/generation-start/failure-recovery status dispatch use narrow injected-port controllers or domain ports. Export label/filename/download planning, filtered-question export row selection, demo analysis artifact planning, HTML/PDF report download identity, ready/blocked execution plans, modal/export/analysis state patches, and settlement patches are pure-descriptor routed while render/download execution and state application stay parent-owned. | `surveyResultsCachePort.ts` defines concrete cache read/write/list/subscribe signatures with late-bound module lookup; `surveyResultsCacheWriteEligibilityPlan.ts` defines target identities, blocked reasons, payload descriptors, state-patch descriptors, analysis artifact pre-read readiness, and outcome shapes from passed-in values; `surveyResultsAnalysisArtifactCachePorts.ts` defines typed analysis-cache read/write port signatures, cache-target normalization, cache-key derivation, sync read request descriptors, and selected-artifact extraction; `surveyResultsAnalysisArtifactReadController.ts` defines the typed read/skipped/failed result for already-built analysis cache read requests; `surveyResultsAnalysisArtifactMergePort.ts` defines generated analysis artifact normalize/merge calls; `surveyResultsAnalysisGeneratedArtifactCompletionPlan.ts` defines generated artifact usability, cache-write payload descriptors, lifecycle success patch descriptors, stale/missing failure shape, and missing-section metadata from passed-in values; `surveyResultsAnalysisLifecyclePlan.ts` defines analysis artifact target identity, payload descriptors, blocked reasons, lifecycle status/result, cached-ready cleanup, and failure-recovery state patches from passed-in values; `surveyResultsAnalysisLifecycleController.ts` defines the typed status/order dispatch result for already-built lifecycle plans; `surveyResultsExportPlans.ts` defines export label, base filename, generation, and inert download descriptors; `surveyResultsExportRows.ts` defines filtered-question export ID/row descriptors; `surveyResultsDemoAnalysisArtifact.ts` defines demo artifact descriptors; `surveyResultsHtmlReportStatePatches.ts` defines HTML report modal/export/analysis state patch descriptors; `surveyResultsHtmlReportModalDescriptor.ts` defines modal analysis-payload/display descriptors; `surveyResultsHtmlReportDownloadAttempt.ts` defines HTML report blocked/ready download attempt and settlement patch descriptors; `surveyResultsHtmlReportDownloadRequest.ts` defines the report export kind, filename identity, render-option descriptor, and ready/blocked execution-plan descriptor. | Identity/payload matrices, write-readiness matrix, cache port call fidelity/late binding, artifact merge call fidelity/late binding, selected fallback/status coverage, broad-normalization blocker classification, export label/generation/download plan identity, filtered-question export ID/row selection, demo artifact descriptor identity, analysis lifecycle controller fake-port routing, analysis read controller fake-port routing, generated completion plan identity/cache-write/lifecycle descriptors, HTML/PDF report download filename/render-option/execution-plan identity, report blocked/ready, modal/export/analysis state patch, and success/failure settlement descriptors, and analysis artifact success, cached-ready cleanup, blocked/no-op, partial failure/retry, parent status output, stale artifact regeneration, stale cached artifact rejection, typed cache read args, read exception fallback, partial cached payload rejection, no-artifact no-read persistence skips, missing identity no-op, plan-derived failure state patch, and no-download/fetch/decrypt side effects are pinned. | cache normalization, feedback timers, warning/error logging, AI generation, generated-artifact completion execution, report rendering, DOM/PDF capture, download execution, decrypt, route, generation fallback, and concrete state application ports. | Broad persistence still crosses cache normalization, AI/export artifacts, generated-artifact completion execution, and parent feedback lifecycles. Export label/generation/download descriptors, analysis artifact cache read dispatch, analysis artifact cache write dispatch, generated completion planning, analysis artifact cache port/request shapes, analysis artifact pre-read readiness, analysis lifecycle status/order, report download identity/execution-plan descriptors, demo artifact descriptors, HTML report state patch descriptors, filtered-question export row descriptors, cache port calls, artifact merge port calls, and report settlement patch descriptors are controller/pure-plan/port routed; broad results cache normalization is test-pinned only, and HTML report execution is still blocked by report rendering, DOM/PDF capture, file download, and parent `setState` ownership. | Next safe lane: keep cache normalization and HTML/PDF export execution parent-owned; generated-artifact completion should stay pure-plan only until a dispatcher can be proven without owning cache-write execution, AI generation, export/download, route/session, decrypt, or concrete state application. |
| SurveyResults refresh/status and read-only cache orchestration | Manual refresh, queued refresh, selected metadata reads, scoped network reads, fallback write dispatch, view/survey reset state, local-storage poll state, concrete cache access, latest-block reads, fetch-request coalescing, local-storage polling backoff/timers, and queued refresh microtask/frame scheduling now have narrow helper/runtime/port boundaries. | `surveyResultsHelpers.ts`, `surveyResultsBlockNumbers.ts`, `surveyResultsCacheControllerSnapshot.ts`, `surveyResultsQuestionMetadataReadController.ts`, `surveyResultsQuestionNetworkReadController.ts`, `surveyResultsCachePort.ts`, `surveyResultsFetchResponsesRuntime.ts`, `surveyResultsLocalStoragePollingRuntime.ts`, and `surveyResultsQueuedRefreshRuntime.ts` hold the pure/read-only/runtime contract shapes. | Refresh/status no-op/failure sequencing, reset patch identity, local-storage poll patch identity, readiness display, malformed block guard behavior, metadata read, scoped network read, cache port call fidelity/late binding, fetch coalescing, polling min/mid/max backoff, hidden/closed/unmounted polling guards, queued refresh microtask/frame coalescing, and parent shell wiring are pinned. | setState application, cache normalization, manual-refresh target state/polling follow-up, nonce refresh status application, decrypt, export, route, AI generation, generated-artifact completion execution, feedback timers, and result mutation. | The remaining lifecycle still crosses parent state application, route/session state, decrypt/export/AI work, and manual-refresh status application. | Next safe lane: keep state application, route/session mutation, decrypt/export/AI execution, and feedback timers parent-owned unless another runtime slice can be proven with focused fake-port tests. |
