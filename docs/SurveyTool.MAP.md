...
- Current shape: `SurveyQuestions.tsx` is the shared hook/engine shell; `surveyQuestionsRuntimeMethods.tsx` holds the legacy runtime method cluster behind that shell; `SurveyResults.tsx` and `SurveyPileViewMode.tsx` remain independent runtime owners.
- Summary: the runtime is no longer one monolithic file. `SurveyQuestions.tsx` is now a function component backed by `useReducer`, with initial state and reducer logic in `surveyQuestionsState.ts`, behavior-level RTL coverage routed through `surveyQuestionsTestHarness.tsx`, and class-parity refs preserving live props/state reads, instance fields, setState callback ordering, and mount/update cleanup ordering. The extracted `surveyQuestionsRuntimeMethods.tsx` factory owns the legacy decrypt/cache/fetch/retry/route/render/submit method cluster and returns the same named methods back to the engine facade, so pile strategies and RTL harnesses keep their existing call-time seams while the shell stays below the size ratchet. `SurveyPileViewMode.tsx` no longer subclasses `SurveyQuestions`; it is a typed pile-mode function wrapper that attaches the pile runtime strategy to the shared SurveyQuestions engine through an explicit dynamic engine boundary. Question/survey chain reads, submit broadcast, registry lookups, sponsored access checks, submitted-payload sanitization, and Arweave question-link normalization route through domain ports. `SurveyResults.tsx` is now a function component backed by `useReducer`, with initial state and reducer logic in `surveyResultsState.ts` and behavior-level RTL coverage routed through `surveyResultsTestHarness.tsx`; PRD 651 SV-R slices moved question-mode cache normalization, demo question metadata fallback, instance-field initialization, runtime path/signature/response accessors, manual-refresh target state/follow-up application, browser download execution, HTML/PDF report render/export execution, AI analysis-section calls, locked encrypted-field normalization/signatures, locked-response envelope decrypt, CSV/JSON data-export runtime orchestration, HTML report model/cache runtime orchestration, CSV/JSON export payload builders, HTML report count/question-row shaping, analysis response-row shaping, analysis segment-dimension shaping, HTML snapshot assembly, locked-response row selection, locked-gate detail shaping, local-storage polling decision/runtime planning, survey/question hydration runtime orchestration, render-surface prop assembly, demo alternate-results prop assembly, generated analysis completion cache-write dispatch, component mount/update lifecycle dispatch, and dead filter-bookmark glue removal behind focused controllers/domain ports, reducing the file to 2358 lines. It still owns export status side effects, nonce refresh status application, warning/error logging, concrete state application, final generated-artifact lifecycle state application, and result mutation.
...
SurveyTool.tsx  [top-level wrapper]
  -> SurveySelector.tsx  [survey/questions selector + filter + results]
     ...
     -> SurveyQuestions.tsx  [shared full response runtime]
        -> surveyQuestionsRuntimeMethods.tsx  [legacy decrypt/cache/fetch/retry/render/submit method cluster returned to the engine facade]
     ...
     -> SurveyResults.tsx  [survey/question results runtime and execution owner]
        ...
        -> surveyResultsDemoQuestionFallback.ts  [built-in demo metadata fallback]
        -> surveyResultsRuntimeHelpers.ts  [runtime path/signature/response helpers]
        -> surveyResultsInstanceFields.ts  [instance-field bag initialization]
        ...
        -> surveyResultsHtmlReportRuntime.ts  [HTML report model/cache runtime wiring]
...
| `SurveyQuestions.tsx` | Shared survey/question hook shell | Function component backed by `useReducer` with class-parity refs for live props/state, instance-field bags, setState callback ordering, and mount/update cleanup ordering. Owns engine setup, the top-level refs/reducer bridge, the initial hydration helper slice, lifecycle dispatch, and the compatibility `Object.assign(engine, ...)` facade while delegating the legacy runtime method cluster to `surveyQuestionsRuntimeMethods.tsx`. |
| `surveyQuestionsRuntimeMethods.tsx` | SurveyQuestions runtime method cluster | Factory that receives the shell scope and returns the same named decrypt/cache/fetch/retry/render/submit methods previously declared inside `SurveyQuestions.tsx`, preserving runtime-strategy and Jest spy seams while the shell stays below the large-file ratchet. This remains a transitional legacy cluster: route/display child components, domain ports, submit controllers, single-question bootstrap planning, and decrypt planning listed below still own their extracted responsibilities. |
...
| `SurveyResults.tsx` | Survey/question results runtime and execution owner | Function component backed by `useReducer` with class-parity refs for live props/state, instance-field bags, setState callback ordering, and mount/update cleanup ordering. 2358 lines after the PRD 651 SV-R runtime-adapter exit slice. Owns filter state, export status side effects, survey/question bookmark logging/state application, nonce refresh status application, generated-artifact final state application, concrete state application, warning/error logging, and result mutation behavior while routing hydration, polling, locked-response model/decrypt, lifecycle setup/update/cleanup, and report-surface prop assembly through the narrow runtime adapters listed below. |
...
| `surveyResultsHydrationRuntime.ts` | Results hydration runtime | Runs survey-mode and question-mode cache hydration through injected cache, parse, filter-reapply, and state-application ports while leaving concrete cache APIs, refs, logging, and React state ownership in `SurveyResults` |
| `surveyResultsLocalStoragePollRuntime.ts` | Local-storage poll runtime | Reads parent-provided cache snapshots, latest-block port, fetch snapshot, and stable-cycle counters to apply local poll patches and queue refreshes while timer/backoff ownership stays in `SurveyResults` / `surveyResultsLocalStoragePollingRuntime.ts` |
| `surveyResultsLockedResponsesRuntime.ts` | Locked-response runtime | Builds locked-response keys/models/gate details, applies decrypted overrides, and runs gated decrypt through injected state, session, question metadata, and crypto ports while keeping concrete state application and cache/route ownership in `SurveyResults` |
| `surveyResultsRenderSurface.tsx` | Results render-surface adapter | Assembles cache/readiness, filter/export, locked banner/toggle, demo surface, modal header, and display-panel props for `SurveyResultsReportSurface.tsx` from parent-provided state, refs, and callbacks without owning fetch, decrypt, cache, route, export, or state behavior |
...
| `surveyResultsDataExportRuntime.ts` | Results data-export runtime wiring | Owns CSV/JSON export generator selection, filename patch dispatch, filtered-question export row lookup, and injected browser-download dispatch from parent-provided state/ports while leaving route/session state, cache reads, fetch, decrypt, and concrete React state application in `SurveyResults` |
...
| `surveyResultsHtmlReportRuntime.ts` | HTML report model/cache runtime wiring | Owns report chain/exporter identity, selected-section mapping, report snapshot input assembly, analysis payload input assembly, and analysis artifact cache read/write dispatch through injected ports while leaving AI generation loops, modal actions, route/session state, final lifecycle state application, and concrete React state application in `SurveyResults` |
...
| `surveyResultsDemoQuestionFallback.ts` | Built-in demo metadata fallback | Detects demo question-results buckets with live responses and overlays canonical demo question metadata only for missing or pending on-chain metadata while leaving cache reads, response parsing, route/session state, and concrete state application in `SurveyResults` |
| `surveyResultsRuntimeHelpers.ts` | Results runtime utility helpers | Merges aggregate question IDs, preserves existing group path prefixes, selects read-only network cache buckets, normalizes nonce/filter signatures, compares value signatures, and reads response question metadata while leaving window history mutation, cache reads, measurement, route/session state, and concrete state application in `SurveyResults` |
| `surveyResultsInstanceFields.ts` | Results instance-field initialization | Creates the default mutable instance-field bag and memo placeholders while leaving field mutation, lifecycle ordering, timers, subscriptions, and React state application in `SurveyResults` |
| `surveyResultsLifecycleRuntime.ts` | Results lifecycle runtime | Runs mount setup/cleanup, component-update patch queuing, modal open/close URL decisions, cache-ready refresh reasons, view/survey reset dispatch, nonce tick dispatch, and question-scope reset dispatch through injected ports while leaving concrete `setState`, refs, polling runtimes, and parent callback implementations in `SurveyResults` |
...
| `surveyResultsFilterBookmarkWriteController.ts` | Filter-bookmark injected write dispatch | Retained as the tested filter-bookmark write unit for legacy/planning coverage; `SurveyResults` no longer wires a runtime caller, and filter bookmarking remains owned by `QuestionFilter` |
...
| `surveyResultsBookmarkCacheReadPorts.ts` | Bookmark cache read ports | Builds bookmark cache read identities and selects copied survey/question bookmark lists from typed cache-value records while leaving concrete cache reads, writes, logging, route/session state, and state application in `SurveyResults` |
...
| UserPage cache/decrypt/action boundaries | Analyze, bookmark, cache-refresh clicks, encrypted visibility status-request planning, cache refresh state patch planning, deep-scan cache-refresh carry/loading input composition, render-surface assembly, runtime contracts/constants, and initial state now use injected, pure-plan, or display helpers. | `userPageActionController.ts`, `userPageGateHelpers.ts`, `userPageLoadingStateHelpers.ts`, `userPageRenderSurface.tsx`, `userPageRuntimeTypes.ts`, `userPageInitialState.ts`, and UserPage helper modules define action/gate/readiness/loading/cache-refresh/render contracts from passed-in values. | Cached responses, gated visibility, no-account encrypted visibility inertness, no-account decrypt-click inertness, decrypt-click wiring, stale/refresh boundaries, incomplete cache lane AI/compare disabling, visible gated cached data, encrypted visibility terminal/read-status identities, cache refresh state patch planning, deep-scan refresh carry/loading input planning, cached-analysis refresh boundaries, and UserPage render chrome are pinned. | Real decrypt execution, sponsored access promises, cache reads/writes, bookmark persistence, AI fetch, route mutation, timers, refresh execution, telemetry emission, and state application remain in `UserPage.tsx`. | Cache/decrypt refresh still crosses user visibility, fetch, analysis, sponsored access, and parent state application. | Next safe lane: keep refresh execution and sponsored access parent-owned; only extract pure patch/display plans that delete parent decision code without forwarding broad prop bundles. |
