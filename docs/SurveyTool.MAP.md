...
- Current lengths:
  ...
  - `surveyResultsQuestionMetadataReadController.ts`: **81 lines**
  - `surveyResultsQuestionNetworkReadController.ts`: **311 lines**
...
| `surveyResultsQuestionNetworkReadController.ts` | Scoped question-network cache read status | Reads scoped question buckets through an injected read-only port, merges ready question/response network data, and returns a plain result/memo patch while leaving cache writes/persistence, async fallback reads, fetch, decrypt, export, route, polling, timers, and state application in `SurveyResults` |
