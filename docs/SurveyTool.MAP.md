...
SurveyTool.tsx  [top-level wrapper]
  -> SurveySelector.tsx  [survey/questions selector + filter + results]
     ...
     -> SurveyResults.tsx  [survey/question results runtime]
        ...
        -> surveyResultsCacheWriteEligibilityPlan.ts  [filter, survey/question bookmark, and analysis artifact cache-write eligibility plans]
...
| `surveyResultsCacheWriteEligibilityPlan.ts` | Cache-write eligibility planning | Derives parent-owned filter, survey/question bookmark, and analysis artifact write targets, blocked reasons, payload shapes, invalid-cache warning flags, success-feedback decisions, and state-patch descriptors from passed-in values while leaving cache reads, cache writes/persistence, timers, feedback/state application, AI generation, fetch, decrypt, export, route, polling, and mutation behavior in `SurveyResults` |
