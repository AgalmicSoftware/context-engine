...
- Current lengths:
  ...
  - `surveyResultsFallbackQuestionHelpers.ts`: **43 lines**
...
| `surveyResultsFallbackQuestionHelpers.ts` | Selected-question fallback planning | Builds plain fallback question objects from passed-in selected question IDs and optional metadata while leaving cache reads/writes, memo state, fetch, decrypt, route, export, and state application in `SurveyResults` |
...
| SurveyResults remaining write blockers | Selected-result/status and refresh/status writes still need explicit eligibility and failure/recovery tests before any controller work. Report-analysis/export cache writes remain AI/export-adjacent and too broad for this lane. | Next lane should start with selected-result/status or refresh/status write prep tests and stop before a controller unless identity, payload, and recovery behavior are pinned. |
