...
- **Key config**: `litNetwork` via `LIT_NETWORK.*`
...
const litNodeClient = new LitNodeClient({
  litNetwork: LIT_NETWORK.NagaDev, // example
});
...
- Global hooks: `window.__litHooks` (saveKey/getKey) are set from `MainSite` using `naga-dev` by default.
...
- `/admin` Worker tests include a Lit quick test panel (encrypt + decrypt without Arweave, default `naga-dev`, override via session `lit.network` / `litNetwork`).
- `/new` Session Wizard no longer shows a Lit quick-test panel; it still writes Lit metadata and normalizes legacy `datil-*` network aliases to `naga-*` when persisting metadata.
