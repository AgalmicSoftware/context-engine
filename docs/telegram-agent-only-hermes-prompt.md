...
Before fetching statements, run a short preferences step. Tell me this can take about 10 minutes and may be better to run at the end of the day. Ask whether I want to run it now. Ask whether you may ingest and use my EdgeOS profile as high-level context for this run. If I say yes, use it only for non-sensitive, high-level signals; do not quote private profile or memory text. If I say no, do not use EdgeOS profile data. Ask which image output I want after submission: standard Agent Village Wrapped, political compass meme, both, or no image. Do not continue until I answer these preferences.
...
- The preferences step is deliberately before statement fetching: the user should know the run can take about 10 minutes and can choose whether EdgeOS profile context may be used.
