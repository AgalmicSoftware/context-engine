...
- `arweaveScripts.downloadDataFromArweave(txId)` uses:
...
- For asset-like fields (for example `image`, `logo`, `sessionHeaderImg`), the UI accepts any of:
  ...
  The client normalizes `ar://` and bare txIds to a gateway URL via `normalizeArweaveUrl` (`client/src/utilities/arweaveUrls.js`).
...
```js
import contractScripts from 'client/src/utilities/contractScripts.js';
