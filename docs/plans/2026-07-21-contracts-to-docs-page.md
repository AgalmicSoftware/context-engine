...
4. **FAQ** (collapsed) — ~7 items: crypto experience needed? cost? where's my data? can answers be private? which chain am I on? what's an SBT? who sees results?
5. **Smart Contracts** (kept; `ContractViewer` unchanged) with a new session-context strip above it: `Session: {sessionName || slug || 'Default (general)'} · Chain: {getChainLabelById(resolvedChainId)}`, testid `ce-docs-session-context` (add to both e2eTestIds files). `resolvedChainId` = the already-computed `networkChainId || firstContract.chainId`, falling back to `DEFAULT_CHAIN_ID`.
6. **Prompts** (kept verbatim, collapsed).
