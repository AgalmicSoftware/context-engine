...
- Publish action controls file: `client/src/components/Sessions/SessionPublishActionControls.tsx` (**75 lines**)
- Publish progress panel file: `client/src/components/Sessions/SessionPublishProgressPanel.tsx` (**81 lines**)
...
- Recent extraction note: bounded follow-up work extracted field descriptors, metadata/publish composition, worker panel sections, narrow modal shells, passive wizard chrome/status pieces, the final passive render shell, the intro/status rail, publish action controls, publish bundle fallback controls, publish advanced settings, publish progress/result display panels, the advanced contracts field, the storage profile field, pure `/new` requirements display planning, pure publish readiness/request/progress/metadata-display planning, pure sponsored auto-deploy publish-surface planning, and narrow publish controller slices. `SessionWizard` still owns state derivation, the public surface contract, and low-level publish side effects; `SessionWizardShell.tsx` owns passive final layout/wiring, `SessionWizardIntroStatusRail.tsx` owns passive requirements/status/normal-mode rail placement, `SessionPublishActionControls.tsx` owns publish button/settings affordance rendering from explicit display/readiness props and named publish/settings execution callback props while routing publish/settings callbacks back to the parent, `SessionPublishBundleFallbackPanel.tsx` owns passive sponsored/manual bundle fallback rendering while routing named bundle URL/file callbacks back to the parent, `SessionPublishAdvancedSettingsPanel.tsx` owns passive manual metadata/gas override rendering while routing named manual override callbacks back to the parent, `SessionPublishProgressPanel.tsx` owns passive publish-progress rendering from the existing display plan, `SessionPublishResultLinks.tsx` owns passive metadata/register/session/admin/result-link rendering while routing the admin-copy callback back to the parent, `SessionWizardContractsField.tsx` owns passive contract-row rendering while routing draft updates/modal opens back to the parent, `SessionWizardStorageProfileField.tsx` owns passive storage-profile controls while routing normalized draft patches back to the parent, `sessionWizardRequirementsDisplay.ts` owns pure `/new` requirements banner visibility/status planning, `sessionWizardPublishReadiness.ts` owns pure publish readiness/request/execution/progress/metadata-display planning, `sessionWizardPublishFlow.ts` owns pure sponsored auto-deploy bundle/fallback publish-surface planning, and `sessionWizardPublishController.ts` owns worker auto-deploy dispatch, pending-SBT step sequencing, metadata-upload dispatch around an injected parent upload port, pure publish start preflight and register-step request/preflight/register-args descriptor planning, register-step tx/status boundary, pure register success/failure settlement planning, completion request planning, and successful completion callbacks.
...
- Start in `sessionWizardPublishReadiness.ts` for pure publish readiness, request identity, metadata fallback/display, UI execution-step, and progress-display plans.
...
- Start in `sessionWizardPublishController.ts` for publish step dispatch, publish start preflight, metadata-upload dispatch, register-step request/preflight/register-args descriptor planning, register-step tx/status, register success/failure settlement planning, completion request planning, and completion sequencing that calls injected side-effect ports/callbacks.
...
| Field renderer and advanced metadata fields | 2406-3153 | Recursive field rendering, lock/gate UI, compact header image controls, contracts/storage field handoffs, normal-vs-advanced metadata fields | `renderCompactSessionHeaderField`, `renderSessionHeaderPreviewSurface`, `renderField`, `SessionWizardContractsField`, `SessionWizardStorageProfileField` |
...
| Publish orchestration and deploy helpers | 3603-4149 | Coordinates publish flow, delegates worker auto-deploy, pending-SBT step sequencing, publish start preflight, metadata-upload dispatch, register-step request/preflight/register-args descriptor planning, register-step tx/status callbacks, register success/failure settlement planning, completion request planning, and successful completion callbacks to `sessionWizardPublishController.ts`, handles copy helpers, session/admin URL application, worker deploy inputs, and connected-admin resolution | `handlePublish`, `resolveSessionWizardPublishStartPreflightDescriptor`, `runSessionWizardPublishController`, `runSessionWizardPublishMetadataUploadController`, `resolveSessionWizardRegisterStepRequest`, `resolveSessionWizardRegisterPreflightDescriptor`, `resolveSessionWizardRegisterArgsDescriptor`, `runSessionWizardRegisterStepController`, `resolveSessionWizardRegisterSuccessSettlementDescriptor`, `resolveSessionWizardRegisterFailureSettlementDescriptor`, `appendSessionWizardRegisterTxEntry`, `resolveSessionWizardPublishCompletionRequest`, `runSessionWizardPublishCompletionController`, `handleCopyAdminUrl`, `handleDeployWorker` |
| Worker/resource cards and derived publish UI | 4150-4532 | Worker deploy result handling, config/secrets sync UI, resource secret inputs, contract modal selection, pure requirements display planning, pure sponsored auto-deploy publish-surface planning, and pure publish readiness/progress/metadata-display plan handoff | `updateResourceGate`, `renderResourceInputs`, `renderResourceCard`, `resolveSessionWizardNewSessionRequirementsDisplayState`, `resolveSessionWizardSponsoredPublishSurfaceState`, `resolveSessionWizardPublishUiPlan`, selected contract memoization |
| Final shell handoff | 4534-4701 | Derives and forwards the full `/new` surface state/handlers into the passive shell while retaining parent-owned side effects | `SessionWizardShell`, `export default SessionWizard` |
...
| Passive publish action controls (`SessionPublishActionControls.tsx`) | 1-75 | Renders normal/advanced publish buttons, loading/disabled states, and advanced-settings affordance from explicit display/readiness props and named execution callback props while routing publish/settings callbacks back to the parent without owning publish, upload, deploy, route, or state mutation side effects | `SessionPublishActionControls` |
...
| Passive publish progress (`SessionPublishProgressPanel.tsx`) | 1-81 | Renders publish step progress from `SessionWizardPublishProgressDisplayState` while parent/controller code owns publish step mutation and publish execution | `SessionPublishProgressPanel` |
...
draft + gates + worker state
  ...
  -> sessionWizardPublishReadiness pure readiness/request/progress/metadata-display UI plan
...
| `publishStep` / `publishBusy` | Publish progress state | Feeds `sessionWizardPublishReadiness.ts` progress UI planning; `SessionPublishProgressPanel` consumes the descriptor-owned step/display state through `SessionPublishSummary` |
