...
- Start in `sessionWizardPublishController.ts` for publish step dispatch, metadata-upload dispatch, register-step request planning, register-step tx/status, completion request planning, and completion sequencing that calls injected side-effect ports/callbacks.
...
| Final shell handoff | 4524-4692 | Derives and forwards the full `/new` surface state/handlers into the passive shell while retaining parent-owned side effects | `SessionWizardShell`, `export default SessionWizard` |
