...
- Current length: **4,615 lines**
- Shell file: `client/src/components/Sessions/SessionWizardShell.tsx` (**581 lines**)
...
| Passive render composition (`SessionWizardShell.tsx`) | 1-581 | Renders header, intro/status rail, encryption, metadata, worker, publish, and modal modules through an explicit typed prop contract without owning publish/worker/storage/SBT/wallet side effects | `SessionWizardHeader`, `SessionWizardIntroStatusRail`, `SessionMetadataEditor`, `WorkerPanel`, `SessionPublishSummary`, `SessionWizardModals` |
