...
| Field renderer and advanced metadata fields | 2406-3284 | Recursive field rendering, lock/gate UI, compact header image controls, contracts-field handoff, normal-vs-advanced metadata fields | `renderCompactSessionHeaderField`, `renderSessionHeaderPreviewSurface`, `renderField`, `SessionWizardContractsField` |
...
| Final shell handoff | 4719-4848 | Derives and forwards the full `/new` surface state/handlers into the passive shell while retaining parent-owned side effects | `SessionWizardShell`, `export default SessionWizard` |
