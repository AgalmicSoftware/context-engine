...
- Publish action controls file: `client/src/components/Sessions/SessionPublishActionControls.tsx` (**68 lines**)
...
| Passive publish action controls (`SessionPublishActionControls.tsx`) | 1-68 | Renders normal/advanced publish buttons, loading/disabled states, and advanced-settings affordance while routing publish/settings callbacks back to the parent without owning publish, upload, deploy, route, or state mutation side effects | `SessionPublishActionControls` |
