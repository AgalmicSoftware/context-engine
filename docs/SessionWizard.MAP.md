...
- Publish summary file: `client/src/components/Sessions/SessionPublishSummary.tsx` (**273 lines**)
...
- Start in `SessionPublishSummary.tsx` for sponsored fallback inputs and publish panel composition. Use `SessionPublishActionControls.tsx` for publish button/settings affordance display, `SessionPublishAdvancedSettingsPanel.tsx` for manual metadata/gas override display, `SessionPublishProgressPanel.tsx` for publish-progress display, and `SessionPublishResultLinks.tsx` for generated metadata/register/session/admin/SBT links.
...
- Start in `sessionWizardPublishController.ts` for publish step dispatch, metadata-upload dispatch, register-step tx/status, and completion sequencing that calls injected side-effect ports/callbacks.
...
| Passive publish summary (`SessionPublishSummary.tsx`) | 1-273 | Renders fallback bundle controls and places passive publish action/advanced-settings/progress/result panels while parent owns publish/deploy/upload/register/storage/wallet side effects | `SessionPublishActionControls`, `SessionPublishAdvancedSettingsPanel`, `SessionPublishProgressPanel`, `SessionPublishResultLinks` |
