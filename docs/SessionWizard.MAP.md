...
- Start in `sessionWizardPublishController.ts` for publish step dispatch, metadata-upload dispatch, register-step request planning, register-args descriptor planning, register-step tx/status, register-success settlement planning, completion request planning, and completion sequencing that calls injected side-effect ports/callbacks.
...
| Publish orchestration and deploy helpers | 3603-4140 | Coordinates publish flow, delegates worker auto-deploy, pending-SBT step sequencing, metadata-upload dispatch, register-step request planning, register-args descriptor planning, register-step tx/status callbacks, register-success settlement planning, completion request planning, and successful completion callbacks to `sessionWizardPublishController.ts`, handles copy helpers, session/admin URL application, worker deploy inputs, and connected-admin resolution | `handlePublish`, `runSessionWizardPublishController`, `runSessionWizardPublishMetadataUploadController`, `resolveSessionWizardRegisterStepRequest`, `resolveSessionWizardRegisterArgsDescriptor`, `runSessionWizardRegisterStepController`, `resolveSessionWizardRegisterSuccessSettlementDescriptor`, `resolveSessionWizardPublishCompletionRequest`, `runSessionWizardPublishCompletionController`, `handleCopyAdminUrl`, `handleDeployWorker` |
...
| Final shell handoff | 4525-4689 | Derives and forwards the full `/new` surface state/handlers into the passive shell while retaining parent-owned side effects | `SessionWizardShell`, `export default SessionWizard` |
