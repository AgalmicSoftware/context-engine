...
- Shell file: `client/src/components/Sessions/SessionWizardShell.tsx` (**402 lines**)
...
| Passive render composition (`SessionWizardShell.tsx`) | 1-402 | Renders header, intro/status rail, encryption, metadata, worker, publish, and modal modules without owning publish/worker/storage/SBT/wallet side effects | `SessionWizardHeader`, `SessionWizardIntroStatusRail`, `SessionMetadataEditor`, `WorkerPanel`, `SessionPublishSummary`, `SessionWizardModals` |
