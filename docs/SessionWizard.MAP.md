...
- Current length: **4,633 lines**
- Shell file: `client/src/components/Sessions/SessionWizardShell.tsx` (**582 lines**)
...
| Passive render composition (`SessionWizardShell.tsx`) | 1-582 | Renders header, intro/status rail, encryption, metadata, worker, publish, and modal modules through an explicit typed prop contract without owning publish/worker/storage/SBT/wallet side effects | `SessionWizardHeader`, `SessionWizardIntroStatusRail`, `SessionMetadataEditor`, `WorkerPanel`, `SessionPublishSummary`, `SessionWizardModals` |
